...
An event driven low level language with 85 compiler targets. The same syntax works across all of them, but the code you write is still target specific you use each architecture's own registers, instructions, and calling conventions.

Decretum is written in Rust and includes a parser, a portable bytecode runtime with a register based virtual machine, 85 direct machine code backends, a peephole optimizer, a byte level optimizer, and a wip gui IDE.
...
* **32 bit MCU or RISC (4):** rx, fr, v810, arduino_nano_esp32
...
* **Soviet and Exotic (5):** ural, besm, mir, harvard, mill

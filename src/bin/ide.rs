...
use decretum::{
    Arm7tdmiBuilder, Arm9Builder, BesmBuilder, BlackfinBuilder, BytecodeRuntime, C166Builder,
    Cdc6600Builder, Direct6502Builder, Direct6809Builder, DirectAarch64Builder, DirectAlphaBuilder,
    DirectArmCmBuilder, DirectAvrBuilder, DirectBiosBuilder, DirectCheriBuilder,
    DirectElf32Builder, DirectElfBuilder, DirectIa64Builder, DirectM68kBuilder, DirectMachoBuilder,
    DirectMicroblazeBuilder, DirectMipsBuilder, DirectNios2Builder, DirectOpenriscBuilder,
    DirectPICBuilder, DirectPariscBuilder, DirectPpcBuilder, DirectQuantum8Builder,
    DirectQuantum64Builder, DirectRisCvCheriBuilder, DirectRiscvBuilder, DirectSh2Builder,
    DirectSparcBuilder, DirectTernaryBuilder, DirectUefiBuilder, DirectVliwBuilder,
    DirectVmBuilder, DirectWin32Builder, DirectX86_64Builder, DirectZ80Builder, DlxBuilder,
    ElbrusBuilder, FrBuilder, H8Builder, HarvardBuilder, Hp3000Builder, HuC6280Builder,
    ...
    Mos6501Builder, Msp430Builder, Nec78kBuilder, NecV20Builder, Parser, Pdp8Builder, Pdp11Builder,
    PicoblazeBuilder, PortableBuilder, Ppc740Builder, Ppc970Builder, Program, R8cBuilder,
    Rl78Builder, RxBuilder, S360Builder, SharcBuilder, Tms320Builder, UnivacBuilder, UralBuilder,
    V810Builder, VaxBuilder, Xc800Builder, ZArchBuilder,
};
...
impl TargetClass {
    fn all() -> &'static [TargetClass] {
        &[
            TargetClass::X86,
            TargetClass::IntelEvolution,
            TargetClass::Arm,
            TargetClass::RiscV,
            TargetClass::ClassicRisc,
            TargetClass::FpgaSoftCore,
            TargetClass::Academic,
            TargetClass::EightBitClassic,
            TargetClass::EightBitMcu,
            TargetClass::SixteenBitMcu,
            TargetClass::ThirtyTwoBitMcu,
            TargetClass::Dsp,
            TargetClass::Hitachi68kConsole,
            TargetClass::PdpVaxHp,
            TargetClass::MainframeVintage,
            TargetClass::Vliw,
            TargetClass::MilAero,
            TargetClass::SovietExotic,
            TargetClass::Special,
        ]
    }
...
impl CompileTarget {
    fn class(self) -> TargetClass {
        match self {
            ...
            CompileTarget::I4004 | CompileTarget::I8008 | CompileTarget::I8080 => {
                TargetClass::IntelEvolution
            }
            CompileTarget::ArmCm
            | CompileTarget::Arm7tdmi
            | CompileTarget::Arm9
            | CompileTarget::Aarch64
            | CompileTarget::Macho
            | CompileTarget::Cheri => TargetClass::Arm,
            CompileTarget::RiscV | CompileTarget::RiscV64 | CompileTarget::RiscVCheri => {
                TargetClass::RiscV
            }
            CompileTarget::Mips
            | CompileTarget::Ppc
            | CompileTarget::Ppc740
            | CompileTarget::Ppc970
            | CompileTarget::Sparc
            | CompileTarget::Alpha
            | CompileTarget::Parisc => TargetClass::ClassicRisc,
            CompileTarget::OpenRisc
            | CompileTarget::Nios2
            | CompileTarget::Microblaze
            | CompileTarget::Mico32
            | CompileTarget::Picoblaze => TargetClass::FpgaSoftCore,
            CompileTarget::Mmix | CompileTarget::Dlx | CompileTarget::Lc3 => TargetClass::Academic,
            CompileTarget::C6502
            | CompileTarget::Z80
            | CompileTarget::C6809
            | CompileTarget::M6800
            | CompileTarget::Mos6501 => TargetClass::EightBitClassic,
            CompileTarget::Pic
            | CompileTarget::Avr
            | CompileTarget::Xc800
            | CompileTarget::Nec78k
            | CompileTarget::R8c => TargetClass::EightBitMcu,
            CompileTarget::Msp430
            | CompileTarget::C166
            | CompileTarget::Rl78
            | CompileTarget::H8
            | CompileTarget::M16c
            | CompileTarget::V20 => TargetClass::SixteenBitMcu,
            CompileTarget::Rx | CompileTarget::Fr | CompileTarget::V810 => {
                TargetClass::ThirtyTwoBitMcu
            }
            CompileTarget::Tms320 | CompileTarget::Blackfin | CompileTarget::Sharc => {
                TargetClass::Dsp
            }
            CompileTarget::Sh2
            | CompileTarget::Sh4
            ...
            | CompileTarget::Besm
            | CompileTarget::Mir
            | CompileTarget::Harvard
            | CompileTarget::Mill => TargetClass::SovietExotic,
            CompileTarget::Portable
            | CompileTarget::Vm
            | CompileTarget::Ternary
            | CompileTarget::Quantum8
            | CompileTarget::Quantum64 => TargetClass::Special,
        }
    }
    ...
    fn targets_for_class(class: TargetClass) -> &'static [CompileTarget] {
        match class {
            ...
            TargetClass::RiscV => &[
                CompileTarget::RiscV,
                CompileTarget::RiscV64,
                CompileTarget::RiscVCheri,
            ],
            TargetClass::ClassicRisc => &[
                CompileTarget::Mips,
                CompileTarget::Ppc,
            ...
            TargetClass::Dsp => &[
                CompileTarget::Tms320,
                CompileTarget::Blackfin,
                CompileTarget::Sharc,
            ],
            TargetClass::Hitachi68kConsole => &[
                CompileTarget::Sh2,
...
impl DecretumIde {
    ...
    fn handle_shortcuts(&mut self, ctx: &egui::Context) {
        ...
        let duplicate =
            ctx.input(|i| i.modifiers.command && !i.modifiers.shift && i.key_pressed(egui::Key::D));
                    ...
                    if !has_main {
                        for (i, line) in self.code.lines().enumerate() {
                            if line.trim().starts_with("event ") {
                                ...
                                self.last_error =
                                    "No entry point specified. Use 'entry' directive.".into();
                            }
                        }
                    }
                }
            ...
            match fs::read_to_string(file) {
                Ok(source) => match Parser::parse(&source) {
                    Ok(program) => {
                        ...
                        let out_file =
                            PathBuf::from("build").join(out_name.file_name().unwrap_or_default());
                        results.push(
        ...
        egui::TopBottomPanel::bottom("output_bottom")
            ...
            .show_inside(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    let out_tab = ui
                        .selectable_label(self.show_run_output, "Output")
                        .clicked();
                    let run_tab = ui
                        .selectable_label(!self.show_run_output, "Run Output")
                        .clicked();
                    if out_tab {
                ...
                .show(ui, |ui| {
                    ui.horizontal_top(|ui| {
                        ...
                        for i in 0..line_count {
                            let n = i + 1;
                            let err_char = if self.error_lines.contains(&n) {
                                "⚠"
                            } else {
                                " "
                            };
                            gutter_buf.push_str(&format!(
                                "{:>width$} {}\n",
                                n,
                                err_char,
                                width = digit_count
    ...
    fn run_virtual(&mut self, path: &Path) {
        ...
        match ext {
            "exe" | "efi" => {
                ...
                thread::spawn(move || match Command::new(&path_buf).output() {
                    Ok(output) => {
            ...
            "img" | "bin" => {
                    ...
                    .spawn()
                {
                    ...
                    Err(_) => {
                        self.run_output =
                            "QEMU not found. Install qemu-system-x86_64 to run BIOS images."
                                .to_string();
...
fn dispatch_compile(
    ...
    match target {
        CompileTarget::Pe => PortableBuilder::build_pe(program, out_path).map(|o| {
            format!(
                "PE -> {} + {}",
                o.pe_path.display(),
                o.bytecode_path.display()
            )
        }),
        CompileTarget::Bytecode => PortableBuilder::build_bytecode(program, out_path)
            .map(|o| format!("Bytecode -> {}", o.bytecode_path.display())),
        CompileTarget::Bios16 => DirectBiosBuilder::build_boot_image(program, out_path).map(|o| {
            format!(
                "BIOS16 -> {} ({} sectors)",
                o.image_path.display(),
                o.sectors_loaded
            )
        }),
        ...
        CompileTarget::RiscV64 => DirectRiscvBuilder::build_bin(program, out_path).map(|o| {
            format!(
                "RISC-V 64 -> {} ({} bytes)",
                o.bin_path.display(),
                o.bin_size
        ...
        CompileTarget::RiscVCheri => {
            DirectRisCvCheriBuilder::build_bin(program, out_path).map(|o| {
                format!(
                    "RISC-V CHERI -> {} ({} bytes)",
                    o.bin_path.display(),
                    o.bin_size

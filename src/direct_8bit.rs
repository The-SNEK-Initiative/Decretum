...
fn e8enc(i: &E8Inst, arch: &str, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    Ok(match i {
        E8Inst::Label(_) => vec![],
        E8Inst::Bytes(b) => b.clone(),
        E8Inst::OpR(mode, op1, op2) => match arch {
            "6502" => vec![*op1 as u8],
            "z80" => match mode {
                0 => vec![*op1 as u8],
                1 => vec![0xCB, *op1 as u8],
                _ => vec![0xDD, *op1 as u8],
            },
            "6809" => vec![*op1 as u8, *op2 as u8],
            _ => vec![0],
        },
        E8Inst::OpI(imm1, imm2) => match arch {
            "6502" => vec![*imm1 as u8],
            "z80" => vec![*imm1 as u8],
...
fn e8parse(t: &str, arch: &str) -> Result<E8Inst, String> {
    ...
    if t.ends_with(':') {
        return Ok(E8Inst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        "nop" => match arch {
            "6502" => Ok(E8Inst::OpI(0xEA, 0)),
            "z80" => Ok(E8Inst::OpI(0x00, 0)),
            "6809" => Ok(E8Inst::OpI(0x12, 0)),
            _ => Err("bad".into()),
        },
        "ret" => Ok(E8Inst::Ret),
...
fn e8(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if t.ends_with(':') {
                items.push(E8Inst::Label(format!(
                    "{}.{}",
                    b.name,
                    t[..t.len() - 1].trim()
                )));
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(E8Inst::Label(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(E8Inst::Label(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" || t == "rts" {
                items.push(E8Inst::Ret);
                continue;
            }
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let E8Inst::Branch(ref mut l, _) = items[prev] {
                    *l = elif_lbl.clone();
                }
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                ...
                b.push(0);
                items.push(E8Inst::Label(format!("__data_{name}")));
                items.push(E8Inst::Bytes(b));
            }
            DataDecl::Scalar { name, width, value } => {
                items.push(E8Inst::Label(format!("__data_{name}")));
                items.push(E8Inst::Bytes(match width {
            ...
            DataDecl::Buffer { name, size } => {
                items.push(E8Inst::Label(format!("__data_{name}")));
                items.push(E8Inst::Bytes(vec![0u8; *size]));
            }
        }
    }
    ...
    for i in &items {
        match i {
            ...
            E8Inst::Bytes(b) => o += b.len() as u32,
            E8Inst::OpR(_, _, _) => o += if arch == "6809" { 2 } else { 1 },
            E8Inst::OpI(_, _) => o += if arch == "6809" { 2 } else { 1 },
            E8Inst::Branch(_, _) => o += 2,
            E8Inst::Jmp(_) => o += 3,
            E8Inst::Ret => o += 1,
        }
    }
    ...
    for i in &items {
        match i {
            E8Inst::Label(_) => {}
            E8Inst::Bytes(b) => bin.extend_from_slice(b),
            _ => bin.extend_from_slice(&e8enc(i, arch, bin.len() as u32, &lm)?),
        }
    }
    Ok(bin)
}

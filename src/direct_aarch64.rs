...
impl DirectAarch64Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Aarch64BuildOutput, String> {
        if p.target != "aarch64" {
            return Err(format!("need aarch64, got '{}'", p.target));
        }
        let k = ab_assemble(p)?;
        std::fs::write(out, &k).map_err(|e| e.to_string())?;
        Ok(Aarch64BuildOutput {
...
enum AInst {
    ...
    MovImm(AReg, u64),
    MovReg(AReg, AReg),
    AddRRR(AReg, AReg, AReg),
    SubRRR(AReg, AReg, AReg),
    AddImm(AReg, AReg, u32),
    SubImm(AReg, AReg, u32),
    LdrPc(AReg, String),
    StrPc(AReg, String),
    LdrOff(AReg, AReg, i16),
    StrOff(AReg, AReg, i16),
    B(String),
    Bl(String),
    Br(AReg),
    Ret,
...
fn aenc(i: &AInst, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    Ok(match i {
        ...
        AInst::LdrOff(d, n, i) => {
            aw(0xF9400000 | (((*i as u32) >> 3) << 10) | (arn(*n) << 5) | arn(*d))
        }
        AInst::StrOff(d, n, i) => {
            aw(0xF9000000 | (((*i as u32) >> 3) << 10) | (arn(*n) << 5) | arn(*d))
        }
...
fn alower(s: &str) -> Result<AInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(AInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        "mov" if v.len() == 2 => {
            if let Ok(imm) = v[1].parse::<u64>() {
                Ok(AInst::MovImm(gr(v[0])?, imm))
            } else {
                Ok(AInst::MovReg(gr(v[0])?, gr(v[1])?))
            }
        }
        "add" if v.len() == 3 => {
            let d = gr(v[0])?;
            ...
            if let Ok(imm) = v[2].parse::<u32>() {
                Ok(AInst::AddImm(d, n, imm))
            } else {
                Ok(AInst::AddRRR(d, n, gr(v[2])?))
            }
        }
        "sub" if v.len() == 3 => {
            let d = gr(v[0])?;
            let n = gr(v[1])?;
            if let Ok(imm) = v[2].parse::<u32>() {
                Ok(AInst::SubImm(d, n, imm))
            } else {
                Ok(AInst::SubRRR(d, n, gr(v[2])?))
            }
        }
        "cmp" if v.len() == 2 => {
            let n = gr(v[0])?;
            if let Ok(imm) = v[1].parse::<u32>() {
                Ok(AInst::CmpImm(n, imm))
            } else {
                Ok(AInst::CmpRR(n, gr(v[1])?))
            }
        }
        ...
        "ldr" if v.len() == 2 => {
            ...
            if mem.contains('[') {
                ...
                Ok(AInst::LdrPc(rd, mem.to_string()))
            }
        }
        "str" if v.len() == 2 => {
            let rd = gr(v[0])?;
            ...
            if mem.contains('[') {
                ...
                Ok(AInst::StrPc(rd, mem.to_string()))
            }
        }
...
fn ab_assemble(p: &Program) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                is.push(AInst::Bl(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                is.push(AInst::Bl(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" {
                is.push(AInst::Ret);
            ...
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = match cond_str.trim().parse::<u64>() {
                    Ok(n) => {
                        is.push(AInst::MovImm(AReg::X10, n));
                        AReg::X10
                    }
                    Err(_) => {
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let AInst::Bcc(ref mut label, _) = is[*prev] {
    ...
    for d in &p.data {
        match d {
            ...
            DataDecl::Buffer { name, size } => {
                is.push(AInst::Label(format!("__data_{}", name)));
                is.push(AInst::Bytes(vec![0u8; *size]));
            }
        }
    }
    ...
    for i in &is {
        match i {
            AInst::Label(_) => {}
            AInst::Bytes(b) => bin.extend_from_slice(b),
            _ => bin.extend_from_slice(&aenc(i, bin.len() as u32, &lm)?),
        }
    }
    Ok(bin)
}

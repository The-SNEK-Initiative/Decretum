...
impl DirectArmCmBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<ArmCmBuildOutput, String> {
        if p.target != "armcm" {
            return Err(format!("need armcm, got '{}'", p.target));
        }
        let k = assemble(p)?;
...
fn bcc(l: &str, c: u8, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    ...
    if r < -256 || r > 254 {
        return Err("bcc out of range".into());
    }
...
fn enc(i: &Inst, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    Ok(match i {
        ...
        Inst::MovImm(r, v) => {
            let d = rn(*r);
            if d <= 7 {
                w16(0x2000 | ((*v as u16) << 8) | ((d as u16) << 4))
            } else {
                return Err("mov imm hi reg n/a".into());
            }
        }
        Inst::MovReg(d, s) => {
            let dd = rn(*d);
            let ss = rn(*s);
            w16(0x4600 | ((ss as u16) << 3) | (dd as u16))
        }
        Inst::AddRRR(d, n, m) => {
            let dd = rn(*d);
            let nn = rn(*n);
            let mm = rn(*m);
            w16(0x1800 | ((mm as u16) << 6) | ((nn as u16) << 3) | (dd as u16))
        }
        Inst::AddImm(r, v) => {
            let d = rn(*r);
            w16(0x3000 | ((d as u16) << 8) | (*v as u16))
        }
        Inst::SubRRR(d, n, m) => {
            let dd = rn(*d);
            let nn = rn(*n);
            let mm = rn(*m);
            w16(0x1A00 | ((mm as u16) << 6) | ((nn as u16) << 3) | (dd as u16))
        }
        Inst::SubImm(r, v) => {
            let d = rn(*r);
            w16(0x3800 | ((d as u16) << 8) | (*v as u16))
        }
        Inst::LdrPc(r, v) => {
            let d = rn(*r);
            w16(0x4800 | ((d as u16) << 8) | (*v as u16))
        }
        Inst::LdrSp(r, v) => {
            let d = rn(*r);
            w16(0x9800 | ((d as u16) << 8) | (*v as u16))
        }
        Inst::StrSp(r, v) => {
            let d = rn(*r);
            w16(0x9000 | ((d as u16) << 8) | (*v as u16))
        }
        Inst::Push(rl) => {
            let mut m = 0u16;
            for r in rl {
            ...
            w16(0xB400 | (if m & (1 << 14) != 0 { 0x100 } else { 0 }) | (m & 0xFF))
        }
        Inst::Pop(rl) => {
            let mut m = 0u16;
            for r in rl {
            ...
            w16(0xBC00 | (if m & (1 << 15) != 0 { 0x100 } else { 0 }) | (m & 0xFF))
        }
        Inst::B(l) => {
            let t = *lm.get(l).ok_or("unknown")?;
            let r = (t.wrapping_sub(off + 4) as i32);
            w16(0xE000 | (((r as u16) >> 1) & 0x7FF))
        }
        Inst::Bl(l) => {
            let t = *lm.get(l).ok_or("unknown")?;
            let r = t.wrapping_sub(off + 4) as i32;
        ...
        Inst::CmpRR(n, m) => {
            ...
            w16(0x4280
                | ((mm as u16) << 6)
                | ((nn as u16) << 3)
                | if nn > 7 || mm > 7 { 0x40 } else { 0 })
        }
        Inst::CmpImm(n, v) => {
            let nn = rn(*n);
            w16(0x2800 | ((nn as u16) << 8) | (*v as u16))
        }
        Inst::Nop => w16(0xBF00),
        Inst::Wfi => w16(0xBF30),
...
fn lower(s: &str) -> Result<Inst, String> {
    ...
    match m {
        "mov" if v.len() == 2 => {
            if let Ok(imm) = v[1].parse::<u8>() {
                Ok(Inst::MovImm(gr(v[0])?, imm))
            } else {
                Ok(Inst::MovReg(gr(v[0])?, gr(v[1])?))
            }
        }
        "add" if v.len() == 3 => {
            let d = gr(v[0])?;
            let n = gr(v[1])?;
            if let Ok(imm) = v[2].parse::<u8>() {
                Ok(Inst::AddImm(d, imm))
            } else {
                Ok(Inst::AddRRR(d, n, gr(v[2])?))
            }
        }
        "sub" if v.len() == 3 => {
            let d = gr(v[0])?;
            let n = gr(v[1])?;
            if let Ok(imm) = v[2].parse::<u8>() {
                Ok(Inst::SubImm(d, imm))
            } else {
                Ok(Inst::SubRRR(d, n, gr(v[2])?))
            }
        }
        "cmp" if v.len() == 2 => {
            let n = gr(v[0])?;
            if let Ok(imm) = v[1].parse::<u8>() {
                Ok(Inst::CmpImm(n, imm))
            } else {
                Ok(Inst::CmpRR(n, gr(v[1])?))
            }
        }
        "push" => {
            let inner = r.trim().trim_matches(|c| c == '{' || c == '}' || c == ' ');
            let regs: Result<Vec<Reg>, _> = inner.split(',').map(|s| gr(s.trim())).collect();
            Ok(Inst::Push(regs?))
        }
        "pop" => {
            let inner = r.trim().trim_matches(|c| c == '{' || c == '}' || c == ' ');
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(Inst::Bl(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(Inst::Bl(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" {
                items.push(Inst::Pop(vec![Reg::Pc]));
                continue;
            }
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = match cond_str.trim().parse::<u8>() {
                    Ok(n) => {
                        items.push(Inst::MovImm(Reg::R0, n));
                        Reg::R0
                    }
                    Err(_) => {
                        ...
                        if rn(r) > 7 {
                            items.push(Inst::MovReg(Reg::R0, r));
                            Reg::R0
                        } else {
                            r
                        }
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let Inst::Beq(ref mut label) = items[*prev] {
            ...
            if let Some(cond_str) = t.strip_prefix("while ") {
                let reg = match cond_str.trim().parse::<u8>() {
                    ...
                    Err(_) => {
                        let reg_str = cond_str.trim();
                        let r = rp(reg_str)
                            .ok_or_else(|| format!("unknown register '{}' for while", reg_str))?;
                        if rn(r) > 7 {
                            items.push(Inst::MovReg(Reg::R0, r));
                            Reg::R0
                        } else {

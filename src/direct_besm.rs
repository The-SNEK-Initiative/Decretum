...
impl BesmBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<BesmBuildOutput, String> {
        if p.target != "besm" {
            return Err(format!("need 'besm'"));
        }
        let mut bin: Vec<u8> = Vec::new();
        let w48 = |v: u64| -> Vec<u8> {
            let b = v.to_be_bytes();
            b[2..8].to_vec()
        };
        struct CfFrame {
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0, 0, 0, 0x06, 0, 0]);
                    continue;
                }
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0, 0, 0, 0, 0, 0]);
                    continue;
                }
                if let Some(r) = t.strip_prefix("if ") {
                    let _ = r.trim();
                    let l = format!("_cf{}", cf_counter);
                    cf_counter += 1;
                    let p = bin.len();
                    bin.extend(w48((0x09u64 << 30) | (0)));
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    let tw = ((bin.len() + 6) / 6) as u64;
                    let mut vb = [0u8; 8];
                    vb[2..8].copy_from_slice(&bin[prev..prev + 6]);
                    let v = u64::from_be_bytes(vb);
                    let oc = (v >> 30) & 0x3F;
                    let nv = (oc << 30) | (tw & 0x3FFF);
                    let nb = nv.to_be_bytes();
                    bin[prev..prev + 6].copy_from_slice(&nb[2..8]);
                    let up = bin.len();
                    bin.extend(w48((0x07u64 << 30) | (0)));
                    f.br_indices.push(up);
                    let cp = bin.len();
                    bin.extend(w48((0x09u64 << 30) | (0)));
                    f.br_indices.push(cp);
                    continue;
                }
                if t == "else" {
                ...
                if t == "endif" {
                    ...
                    for &idx in &f.br_indices {
                        let mut vb = [0u8; 8];
                        vb[2..8].copy_from_slice(&bin[idx..idx + 6]);
                        let v = u64::from_be_bytes(vb);
                        let oc = (v >> 30) & 0x3F;
                        let nv = (oc << 30) | (tw & 0x3FFF);
                        let nb = nv.to_be_bytes();
                        bin[idx..idx + 6].copy_from_slice(&nb[2..8]);
                    }
                    continue;
                }
                ...
                if t == "endwhile" {
                    ...
                    let sw = (f.start_pos / 6) as u64;
                    bin.extend(w48((0x07u64 << 30) | (sw)));
                    let tw = (bin.len() / 6) as u64;
                    for &idx in &f.br_indices {
                        let mut vb = [0u8; 8];
                        vb[2..8].copy_from_slice(&bin[idx..idx + 6]);
                ...
                bin.extend(match m {
                    ...
                    "ld" => w48((0x05u64 << 30) | (ap(0) & 0x3FFF)),
                    "st" => w48((0x06u64 << 30) | (ap(0) & 0x3FFF)),

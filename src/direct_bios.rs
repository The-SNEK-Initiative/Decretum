    ...
    fn assemble(program: &Program) -> Result<Vec<u8>, String> {
            ...
            for raw_line in &block.lines {
                ...
                if let Some(cond_str) = line.strip_prefix("if ") {
                    let reg = match cond_str.trim().parse::<i64>() {
                        Ok(n) => {
                            items.push(Item::Inst(Inst::MovReg16Imm(
                                Reg16::Ax,
                                Imm16::Value(n as i16),
                            )));
                            Reg16::Ax
                        }
                        Err(_) => parse_reg16(cond_str.trim())?,
                    };
        ...
        // Peephole: NOP compression + dead code elimination
        {
            ...
            while i < items.len() {
                ...
                let is_term = |item: &Item| {
                    matches!(
                        item,
                        Item::Inst(Inst::Jmp(_) | Inst::Ret | Inst::Hlt | Inst::Int(_))
                    )
                };

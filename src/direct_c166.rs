...
impl C166Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<C166BuildOutput, String> {
        if p.target != "c166" {
            return Err(format!("need 'c166'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    ...
                    "mov" | "ld" if a.len() == 2 => {
                        vec![0x04u8 | ((rp(0) & 0xF) << 4) | (rp(1) & 0xF), 0]
                    }

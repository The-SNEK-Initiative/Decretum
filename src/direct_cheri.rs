...
impl DirectCheriBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<CheriBuildOutput, String> {
        if p.target != "cheri" {
            return Err(format!("need cheri, got '{}'", p.target));
        }
        let k = cr_assemble(p)?;
...
enum CInst {
    ...
    MovImm(CReg, u64),
    MovReg(CReg, CReg),
    AddRRR(CReg, CReg, CReg),
    SubRRR(CReg, CReg, CReg),
    AddImm(CReg, CReg, u32),
    SubImm(CReg, CReg, u32),
    B(String),
    Bl(String),
    Cbz(String, CReg),
    Ret,
    Nop,
    CSetAddr(CReg, CReg, u64),
    CSetBounds(CReg, CReg, u64),
    CAndPerm(CReg, CReg, u32),
    CLd(CReg, CReg, i16),
    CSt(CReg, CReg, i16),
    CCall(CReg, CReg),
    CRet(CReg),
...
fn cenc(i: &CInst, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    ...
    Ok(match i {
        ...
        CInst::CLd(cd, cb, _imm) => cw(0xFC000000 | (crn(*cb) << 5) | crn(*cd)),
        CInst::CSt(cs, cb, _imm) => cw(0xFC000000 | (crn(*cs) << 16) | (crn(*cb) << 5)),
...
fn clower(s: &str) -> Result<CInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(CInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        "mov" if v.len() == 2 => {
            if let Ok(imm) = v[1].parse::<u64>() {
                Ok(CInst::MovImm(gr(v[0])?, imm))
            } else {
                Ok(CInst::MovReg(gr(v[0])?, gr(v[1])?))
            }
        }
        "add" if v.len() == 3 => {
            let d = gr(v[0])?;
            let n = gr(v[1])?;
            if let Ok(imm) = v[2].parse::<u32>() {
                Ok(CInst::AddImm(d, n, imm))
            } else {
                Ok(CInst::AddRRR(d, n, gr(v[2])?))
            }
        }
        "sub" if v.len() == 3 => {
            let d = gr(v[0])?;
            let n = gr(v[1])?;
            if let Ok(imm) = v[2].parse::<u32>() {
                Ok(CInst::SubImm(d, n, imm))
            } else {
                Ok(CInst::SubRRR(d, n, gr(v[2])?))
            }
        }
        ...
        "ret" => Ok(CInst::Ret),
        "nop" => Ok(CInst::Nop),
        "csetaddr" => Ok(CInst::CSetAddr(
            gr(v[0])?,
            gr(v[1])?,
            v[2].parse().map_err(|_| "bad addr")?,
        )),
        "csetbounds" => Ok(CInst::CSetBounds(
            gr(v[0])?,
            gr(v[1])?,
            v[2].parse().map_err(|_| "bad sz")?,
        )),
        "candperm" => Ok(CInst::CAndPerm(
            gr(v[0])?,
            gr(v[1])?,
            v[2].parse().map_err(|_| "bad perms")?,
        )),
...
fn cr_assemble(p: &Program) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                is.push(CInst::Bl(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                is.push(CInst::Bl(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" {
                is.push(CInst::Ret);
            ...
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = crp(cond_str.trim())
                    .ok_or_else(|| format!("unknown reg '{}'", cond_str.trim()))?;
            ...
            if t == "endif" {
                ...
                match frame.kind {
                    CfKind::While => return Err("endif without matching if".into()),
                    _ => {}
                }
            ...
            if t == "endwhile" {
                ...
                match frame.kind {
                    CfKind::If => return Err("endwhile without matching while".into()),
                    _ => {}
                }
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                ...
                b.push(0);
                is.push(CInst::Label(format!("__data_{}", name)));
                is.push(CInst::Bytes(b));
            }
            ...
            DataDecl::Buffer { name, size } => {
                is.push(CInst::Label(format!("__data_{}", name)));
                is.push(CInst::Bytes(vec![0u8; *size]));
            }
        }
    }
    ...
    for i in &is {
        match i {
            CInst::Label(_) => {}
            CInst::Bytes(b) => bin.extend_from_slice(b),
            _ => bin.extend_from_slice(&cenc(i, bin.len() as u32, &lm)?),
        }
    }
    Ok(bin)
}

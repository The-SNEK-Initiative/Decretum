...
pub struct HuC6280BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct HuC6280Builder;
pub struct V810BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct V810Builder;
pub struct Arm7tdmiBuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct Arm7tdmiBuilder;
pub struct Arm9BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct Arm9Builder;
pub struct Ppc740BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct Ppc740Builder;
pub struct Ppc970BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
...
impl HuC6280Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<HuC6280BuildOutput, String> {
        if p.target != "huc6280" {
            return Err(format!("need 'huc6280', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
            ...
            for l in &b.lines {
                ...
                if t == "endif" {
                    ...
                    for &pos in &frame.br_indices {
                        if bin[pos] == 0xF0 {
                            pi_rel(&mut bin, pos, target)
                        } else {
                            pi_abs(&mut bin, pos, target)
                        }
                    }
                    continue;
                }
                ...
                bin.extend(match m {
                    ...
                    "bpl" => vec![0x10, ap(0)],
                    "bmi" => vec![0x30, ap(0)],
                    "jmp" => vec![0x4C, ap(0), ap(1)],
                    "jsr" => vec![0x20, ap(0), ap(1)],
                    "tam" | "tma" => vec![0xEA],
                    "nop" => vec![0xEA],
                    "rts" => vec![0x60],
                    "brk" => vec![0x00],
...
impl V810Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<V810BuildOutput, String> {
        if p.target != "v810" {
            return Err(format!("need 'v810', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x40, 0, 0, 0]);
                    continue;
                }
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0x80, 0, 0, 0]);
                    continue;
                }
                let parts: Vec<&str> = t
                    .splitn(4, |c: char| c == ' ' || c == '\t')
                ...
                let pi = |bin: &mut Vec<u8>, pos: usize, target: usize| {
                    let instr =
                        u32::from_be_bytes([bin[pos], bin[pos + 1], bin[pos + 2], bin[pos + 3]]);
                ...
                if let Some(cond) = t.strip_prefix("elif ") {
                    ...
                    let last = frame.br_indices.pop().ok_or("no branch".to_string())?;
                    let here = bin.len();
                    pi(&mut bin, last, here);
                    let bra = bin.len();
                    bin.extend(&be(0x82u32 << 26 | 0));
                    frame.br_indices.push(bra);
                    bin.extend(&be(0x22u32 << 26 | (rn & 0x1F) << 21 | 0 << 16 | 0 << 11));
                    let beq = bin.len();
                    bin.extend(&be(0x02u32 << 26 | 0));
                ...
                bin.extend(match m {
                    "add" | "addu" => be(0x20u32 << 26
                        | (rp(0) & 0x1F) << 21
                        | (rp(1) & 0x1F) << 16
                        | (rp(2) & 0x1F) << 11),
                    "sub" | "subu" => be(0x21u32 << 26
                        | (rp(0) & 0x1F) << 21
                        | (rp(1) & 0x1F) << 16
                        | (rp(2) & 0x1F) << 11),
                    "cmp" => be(0x22u32 << 26
...
impl Arm7tdmiBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Arm7tdmiBuildOutput, String> {
        if p.target != "arm7tdmi" {
            return Err(format!("need 'arm7tdmi', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0xEB, 0, 0, 0]);
                    continue;
                }
                if t == "ret" {
                    bin.extend(&[0xE1, 0x2F, 0xFF, 0x1E]);
                    continue;
                }
                if t == "hlt" {
                    bin.extend(&[0xE0, 0, 0, 0]);
                    continue;
                }
                let parts: Vec<&str> = t
                    .splitn(4, |c: char| c == ' ' || c == '\t')
                ...
                let pi = |bin: &mut Vec<u8>, pos: usize, target: usize| {
                    let instr =
                        u32::from_le_bytes([bin[pos], bin[pos + 1], bin[pos + 2], bin[pos + 3]]);
                ...
                bin.extend(match m {
                    "add" => {
                        be(0xE0800000 | (rp(0) & 0xF) << 16 | (rp(1) & 0xF) << 12 | (rp(2) & 0xF))
                    }
                    "sub" => {
                        be(0xE0400000 | (rp(0) & 0xF) << 16 | (rp(1) & 0xF) << 12 | (rp(2) & 0xF))
                    }
                    "mov" => be(0xE1A00000 | (rp(0) & 0xF) << 12 | (rp(1) & 0xF)),
                    "cmp" => be(0xE1500000 | (rp(0) & 0xF) << 16 | (rp(1) & 0xF) << 12),
                    "ldr" | "ld" => {
                        be(0xE5900000 | (rp(0) & 0xF) << 12 | (rp(1) & 0xF) << 16 | ap(2) & 0xFFF)
                    }
                    "str" | "st" => {
                        be(0xE5800000 | (rp(0) & 0xF) << 12 | (rp(1) & 0xF) << 16 | ap(2) & 0xFFF)
                    }
...
impl Arm9Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Arm9BuildOutput, String> {
        if p.target != "arm9" {
            return Err(format!("need 'arm9', got '{}'", p.target));
        }
        match Arm7tdmiBuilder::build_bin(p, out) {
            Ok(o) => Ok(Arm9BuildOutput {
                bin_path: o.bin_path,
                bin_size: o.bin_size,
            }),
            Err(e) => Err(e),
...
impl Ppc740Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Ppc740BuildOutput, String> {
        if p.target != "ppc740" {
            return Err(format!("need 'ppc740', got '{}'", p.target));
        }
        let r = crate::direct_risc::DirectPpcBuilder::build_bin(
            &Program {
                target: "ppc".into(),
                ..p.clone()
            },
...
impl Ppc970Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Ppc970BuildOutput, String> {
        if p.target != "ppc970" {
            return Err(format!("need 'ppc970', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x48, 0, 0, 0x01]);
                    continue;
                }
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0x4E, 0x80, 0x00, 0x20]);
                    continue;
                }
                let parts: Vec<&str> = t
                    .splitn(4, |c: char| c == ' ' || c == '\t')

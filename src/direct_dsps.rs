...
pub struct Tms320BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct Tms320Builder;
pub struct BlackfinBuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct BlackfinBuilder;
pub struct SharcBuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
...
impl Tms320Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Tms320BuildOutput, String> {
        if p.target != "tms320" {
            return Err(format!("need 'tms320', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                let args: Vec<&str> = joined
                    ...
                    .collect();
                let ap =
                    |i: usize| args.get(i).and_then(|s| s.parse::<u16>().ok()).unwrap_or(0) as u8;
                let rp = |i: usize| {
                    args.get(i)
                        .and_then(|s| {
...
impl BlackfinBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<BlackfinBuildOutput, String> {
        if p.target != "blackfin" {
            return Err(format!("need 'blackfin', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x5C, 0, 0, 0]);
                    continue;
                }
                if t == "ret" || t == "rts" {
                    bin.extend(&[0x5D, 0, 0, 0]);
                    continue;
                }
                if t == "hlt" {
                    bin.extend(&[0x00, 0, 0, 0]);
                    continue;
                }
                let parts: Vec<&str> = t
                    .splitn(4, |c: char| c == ' ' || c == '\t')
                ...
                bin.extend(match m {
                    ...
                    "add" if args.len() == 3 => {
                        inst(0x10u32 | ((rp(0) & 7) << 8) | ((rp(1) & 7) << 4) | (rp(2) & 7))
                    }
                    "sub" if args.len() == 3 => {
                        inst(0x20u32 | ((rp(0) & 7) << 8) | ((rp(1) & 7) << 4) | (rp(2) & 7))
                    }
                    "mul" | "mpy" => {
                        inst(0x30u32 | ((rp(0) & 7) << 8) | ((rp(1) & 7) << 4) | (rp(2) & 7))
                    }
...
impl SharcBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<SharcBuildOutput, String> {
        if p.target != "sharc" {
            return Err(format!("need 'sharc', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    ...
                    "fld" | "load" => {
                        inst(0x40u32 | ((rp(0) as u32 & 0xF) << 4) | (rp(1) as u32 & 0xF))
                    }
                    "fst" | "store" => {
                        inst(0x50u32 | ((rp(0) as u32 & 0xF) << 4) | (rp(1) as u32 & 0xF))
                    }
                    "add" | "iadd" => inst(
                        0x14u32
                            | ((rp(0) as u32 & 0xF) << 8)

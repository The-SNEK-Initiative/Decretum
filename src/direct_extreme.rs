...
enum EInst {
    Label(String),
    Bytes(Vec<u8>),
    Etype(EReg, EReg, EReg, u32, u32),
    Eimm(EReg, EReg, u32, u32),
    Eret,
    Branch(String, EReg, u32),
}
...
fn eparse(t: &str, arch: &str) -> Result<EInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(EInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        ...
        "li" if v.len() == 2 => {
            let rd = gr(v[0])?;
            let imm: u32 = v[1].parse().map_err(|_| "bad imm")?;
            let op = if arch == "alpha" {
                0x11 << 26
            } else {
                0x34 << 26
            };
        ...
        "add" | "sub" if v.len() == 3 => {
            ...
            let (op, funct) = match (m, arch) {
                ("add", "alpha") => (0x10 << 26, 0x00),
                ("sub", "alpha") => (0x10 << 26, 0x28),
                ("add", "parisc") => (0x08 << 26, 0x028),
                ("sub", "parisc") => (0x08 << 26, 0x228),
...
fn earch(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(EInst::Eimm(EReg::V0, EReg::V0, 0, 0));
                items.push(EInst::Label(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(EInst::Eimm(EReg::V0, EReg::V0, 0, 0));
                items.push(EInst::Label(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" {
                items.push(EInst::Eret);
                continue;
            }

            // if <reg>
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = erp(cond_str.trim())
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let EInst::Branch(ref mut l, ..) = items[prev] {
                    *l = elif_lbl.clone();
                }
    ...
    {
        let nop_op = if arch == "alpha" {
            0x11u32 << 26
        } else {
            0x0Au32 << 26
        };
        let is_nop =
            |i: &EInst| matches!(i, EInst::Eimm(EReg::V0, EReg::V0, 0, op) if *op == nop_op);
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                ...
                b.push(0);
                items.push(EInst::Label(format!("__data_{name}")));
                items.push(EInst::Bytes(b));
            }
            DataDecl::Scalar { name, width, value } => {
                items.push(EInst::Label(format!("__data_{name}")));
                items.push(EInst::Bytes(match width {
            ...
            DataDecl::Buffer { name, size } => {
                items.push(EInst::Label(format!("__data_{name}")));
                items.push(EInst::Bytes(vec![0u8; *size]));
            }
        }
    }
    ...
    for i in &items {
        match i {
            EInst::Label(_) => {}
            EInst::Bytes(b) => bin.extend_from_slice(b),
            _ => bin.extend_from_slice(&eenc(i, arch, bin.len() as u32, &lm)?),
        }
    }
    Ok(bin)
}

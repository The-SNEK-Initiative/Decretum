...
fn frp(s: &str) -> Option<FReg> {
    let s2 = s
        .trim_start_matches('$')
        .trim_start_matches('r')
        .to_lowercase();
    let n = s2.parse::<u8>().ok()?;
    if n <= 31 {
        Some(match n {
            0 => FReg::F0,
            1 => FReg::F1,
...
enum FInst {
    Label(String),
    Bytes(Vec<u8>),
    Op3(FReg, FReg, FReg, u32, u32),
    Op2(FReg, FReg, u32, u32),
    Fret,
    CondBranch(FReg, String),
    Jump(String),
}

fn fenc(i: &FInst, arch: &str) -> Result<Vec<u8>, String> {
    Ok(match i {
        FInst::Label(_) => vec![],
        FInst::Bytes(b) => b.clone(),
        FInst::Op3(rd, rs, rt, op, funct) => {
            let d = frn(*rd);
            let s = frn(*rs);
            let t = frn(*rt);
            match arch {
        ...
        FInst::Fret => match arch {
            "openrisc" => fu4(0x44000001),
            "nios2" => fu4(0xE8080000),
            "microblaze" => fu4(0x98087D60),
            _ => vec![0; 4],
        },
...
fn fparse(t: &str, arch: &str) -> Result<FInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(FInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        ...
        "add" | "sub" if v.len() == 3 => {
            ...
            let (op, funct) = match (m, arch) {
                ("add", "openrisc") => (0x38 << 26, 0x00),
                ("sub", "openrisc") => (0x38 << 26, 0x02),
                ("add", "nios2") => (0x20 << 26, 0x04),
                ("sub", "nios2") => (0x20 << 26, 0x06),
                ("add", "microblaze") => (0x00 << 26, 0x00),
                ("sub", "microblaze") => (0x00 << 26, 0x01),
...
fn fpga(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    items.push(FInst::Op2(FReg::F0, FReg::F0, 0, 0));
    items.push(FInst::Label(format!("__event_{}", p.entry_event)));
    for b in &p.blocks {
        let pr = match b.kind {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(FInst::Op2(FReg::F0, FReg::F0, 0, 0));
                items.push(FInst::Label(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(FInst::Op2(FReg::F0, FReg::F0, 0, 0));
                items.push(FInst::Label(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" {
                items.push(FInst::Fret);
                continue;
            }
            ...
            match fparse(t, arch) {
                Ok(i) => items.push(i),
                Err(_) => return Err(format!("line '{t}'")),
            }
        }
    }
    ...
    // Peephole: NOP compression + dead-code elimination
    {
        ...
        crate::direct_peephole::peephole(
            ...
            |i| match i {
                FInst::Op2(_, _, _, op) => br_ops.contains(op),
                FInst::CondBranch(..) | FInst::Jump(_) => true,
                _ => false,
            },
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                ...
                b.push(0);
                items.push(FInst::Label(format!("__data_{name}")));
                items.push(FInst::Bytes(b));
            }
            DataDecl::Scalar { name, width, value } => {
                items.push(FInst::Label(format!("__data_{name}")));
                items.push(FInst::Bytes(match width {
            ...
            DataDecl::Buffer { name, size } => {
                items.push(FInst::Label(format!("__data_{name}")));
                items.push(FInst::Bytes(vec![0u8; *size]));
            }
        }
    }
    ...
    for i in &items {
        match i {
            ...
            FInst::CondBranch(reg, label) => {
                ...
                match arch {
                    "openrisc" => {
                        bin.extend(&fu4(0x10 << 26
                            | frn(*reg) << 16
                            | frn(FReg::F0) << 11
                            | 0x1E));
                        let disp = ((target as i32 - cur as i32 - 4) / 4) as i32;
                        if disp < 0 || disp > 0x3FFFFFF {
                            return Err("branch out of range".into());
                        }
                        bin.extend(&fu4(0x0C << 26 | (disp as u32 & 0x3FFFFFF)));
                    }
                    "nios2" => {
                        let imm = ((target as i32 - cur as i32 - 4) / 4) as i16;
                        bin.extend(&fu4(0x1C << 26
                            | frn(*reg) << 21
                            | frn(FReg::F0) << 16
                            | (imm as u16 as u32 & 0xFFFF)));
                    }
                    "microblaze" => {
                        let imm = ((target as i32 - cur as i32 - 4) / 4) as i16;
                        bin.extend(&fu4(0x26 << 26
                            | frn(*reg) << 16
                            | (imm as u16 as u32 & 0xFFFF)));
                    }

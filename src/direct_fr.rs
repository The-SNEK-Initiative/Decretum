...
impl FrBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<FrBuildOutput, String> {
        if p.target != "fr" {
            return Err(format!("need 'fr'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x9B, 0, 0, 0]);
                    continue;
                }
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0x9C, 0, 0, 0]);
                    continue;
                }
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    let tgt = (bin.len() + 4) as i32;
                    let d = tgt - (prev as i32 + 4);
                    let du = d as u32;
                    bin[prev + 1] = ((du >> 16) & 0xFF) as u8;
                ...
                if t == "endif" {
                    ...
                    for &idx in &f.br_indices {
                        let d = tgt - (idx as i32 + 4);
                        let du = d as u32;
                        bin[idx + 1] = ((du >> 16) & 0xFF) as u8;

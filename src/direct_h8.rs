...
impl H8Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<H8BuildOutput, String> {
        if p.target != "h8" {
            return Err(format!("need 'h8'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    let tgt = (bin.len() + 2) as i32;
                    let rel = (tgt - (prev as i32 + 2)) as i8 as u8;
                    bin[prev + 1] = rel;
                    let up = bin.len();
                    bin.extend(&[0x40, 0]);
                ...
                if t == "endif" {
                    ...
                    for &idx in &f.br_indices {
                        let rel = (tgt - (idx as i32 + 2)) as i8 as u8;
                        bin[idx + 1] = rel;
                    }
                    continue;
                }
                ...
                if t == "endwhile" {
                    ...
                    let sw = f.start_pos as i32;
                    let up = bin.len();
                    let rel = (sw - (up as i32 + 2)) as i8 as u8;
                    bin.extend(&[0x40, rel]);
                    let tgt = bin.len() as i32;
                    for &idx in &f.br_indices {
                        let rel = (tgt - (idx as i32 + 2)) as i8 as u8;
                ...
                bin.extend(match m {
                    ...
                    "inc" => vec![0x14u8 | ((rp(0) & 7) << 4), ap(0)],
                    "dec" => vec![0x1Cu8 | ((rp(0) & 7) << 4), ap(0)],
                    "cmp" => vec![0x30u8 | ((rp(0) & 7) << 4) | (rp(1) & 7), ap(1)],
                    "jmp" => vec![0x40u8 | (rp(0) & 3), 0],
                    "jsr" => vec![0xB0, 0],
                    "bne" => vec![0x42, 0],
                    "beq" => vec![0x43, 0],
                    "bra" => vec![0x40, 0],

...
impl HarvardBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<HarvardBuildOutput, String> {
        if p.target != "harvard" {
            return Err(format!("need 'harvard'"));
        }
            ...
            for l in &b.lines {
                ...
                if t == "endif" {
                    ...
                    for &idx in &frame.br_indices {
                        patch_branch(&mut bin, idx, target);
                    }
                    continue;
                }
                ...
                bin.extend(match m {
                    "add" => w(0x1000u16 | (rp(0) << 8) | rp(1)),
                    "sub" => w(0x2000u16 | (rp(0) << 8) | rp(1)),
                    "mul" => w(0x3000u16 | (rp(0) << 8) | rp(1)),
                    "div" => w(0x4000u16 | (rp(0) << 8) | rp(1)),
                    ...
                    "jmp" => w(0xA000u16 | (ap(0) & 0x0FFF)),
                    "call" => w(0xB000u16 | (ap(0) & 0x0FFF)),
                    "ret" => w(0x0000),
                    "nop" => w(0xFFFF),

...
pub fn parse_i386_reg(s: &str) -> Option<I386Reg> {
    Some(match s.to_lowercase().as_str() {
        "eax" | "ax" | "al" => I386Reg::Eax,
        "ecx" | "cx" | "cl" => I386Reg::Ecx,
        "edx" | "dx" | "dl" => I386Reg::Edx,
        "ebx" | "bx" | "bl" => I386Reg::Ebx,
        "esp" | "sp" | "ah" => I386Reg::Esp,
        "ebp" | "bp" | "ch" => I386Reg::Ebp,
        "esi" | "si" | "dh" => I386Reg::Esi,
        "edi" | "di" | "bh" => I386Reg::Edi,
        _ => return None,
    })
}

pub fn reg_num(r: I386Reg) -> u8 {
    r as u8
}
...
pub enum Inst {
    Label(String),
    Bytes(Vec<u8>),
    MovRegImm(I386Reg, i32),
    MovRegReg(I386Reg, I386Reg),
    MovRegMem(I386Reg, String),
    MovMemReg(String, I386Reg),
    AddRegReg(I386Reg, I386Reg),
    SubRegReg(I386Reg, I386Reg),
    AddRegImm(I386Reg, i32),
    SubRegImm(I386Reg, i32),
    XorRegReg(I386Reg, I386Reg),
    AndRegReg(I386Reg, I386Reg),
    OrRegReg(I386Reg, I386Reg),
    IncReg(I386Reg),
    DecReg(I386Reg),
    MulReg(I386Reg),
    DivReg(I386Reg),
    Push(I386Reg),
    Pop(I386Reg),
    CmpRegReg(I386Reg, I386Reg),
    CmpRegImm(I386Reg, i32),
    Jmp(String),
    Call(String),
    Ret,
    Je(String),
...
pub fn encode_inst(
    ...
    match inst {
        ...
        Inst::MovRegReg(d, s) => Ok(vec![0x89, modrm(3, reg_num(*d), reg_num(*s))]),
        ...
        Inst::AddRegImm(r, v) => {
            if *v >= -128 && *v <= 127 {
                Ok(vec![0x83, modrm(3, 0, reg_num(*r)), *v as u8])
            } else {
                let mut b = vec![0x81, modrm(3, 0, reg_num(*r))];
        ...
        Inst::SubRegImm(r, v) => {
            if *v >= -128 && *v <= 127 {
                Ok(vec![0x83, modrm(3, 5, reg_num(*r)), *v as u8])
            } else {
                let mut b = vec![0x81, modrm(3, 5, reg_num(*r))];
                b.extend_from_slice(&v.to_le_bytes());
                Ok(b)
            }
        }
        ...
        Inst::CmpRegImm(r, v) => {
            if *v >= -128 && *v <= 127 {
                Ok(vec![0x83, modrm(3, 7, reg_num(*r)), *v as u8])
            } else {
                let mut b = vec![0x81, modrm(3, 7, reg_num(*r))];
                b.extend_from_slice(&v.to_le_bytes());
                Ok(b)
            }
        }
...
pub fn lower(s: &str) -> Result<Inst, String> {
    ...
    if t.ends_with(':') {
        return Ok(Inst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        "mov" if v.len() == 2 => {
            if let Ok(imm) = v[1].parse::<i32>() {
                return Ok(Inst::MovRegImm(rp(v[0])?, imm));
            }
            if v[1].starts_with('[') || v[0].starts_with('[') {
                if v[0].starts_with('[') {
                    let label = v[0].trim_matches(|c| c == '[' || c == ']');
                    return Ok(Inst::MovMemReg(label.to_string(), rp(v[1])?));
                }
                let label = v[1].trim_matches(|c| c == '[' || c == ']');
                return Ok(Inst::MovRegMem(rp(v[0])?, label.to_string()));
            }
        ...
        "add" | "sub" | "xor" | "and" | "or" if v.len() == 2 => {
            ...
            if let Ok(imm) = v[1].parse::<i32>() {
                Ok(match m {
                    "add" => Inst::AddRegImm(d, imm),
                    "sub" => Inst::SubRegImm(d, imm),
                    _ => return Err("no imm".into()),
                })
            } else {
                let s = rp(v[1])?;
                Ok(match m {
                    "add" => Inst::AddRegReg(d, s),
                    "sub" => Inst::SubRegReg(d, s),
                    "xor" => Inst::XorRegReg(d, s),
                    "and" => Inst::AndRegReg(d, s),
                    "or" => Inst::OrRegReg(d, s),
                    _ => unreachable!(),
                })
            }
        }
        "inc" => Ok(Inst::IncReg(rp(v[0])?)),
        "dec" => Ok(Inst::DecReg(rp(v[0])?)),
        "mul" => Ok(Inst::MulReg(rp(v[0])?)),
        "div" => Ok(Inst::DivReg(rp(v[0])?)),
        "push" => Ok(Inst::Push(rp(v[0])?)),
        "pop" => Ok(Inst::Pop(rp(v[0])?)),
        "cmp" if v.len() == 2 => {
            let a = rp(v[0])?;
            if let Ok(imm) = v[1].parse::<i32>() {
                Ok(Inst::CmpRegImm(a, imm))
            } else {
                Ok(Inst::CmpRegReg(a, rp(v[1])?))
            }
        }
        "jmp" => Ok(Inst::Jmp(v[0].to_string())),
        "call" => Ok(Inst::Call(v[0].to_string())),
        "ret" => Ok(Inst::Ret),
        "je" | "jz" => Ok(Inst::Je(v[0].to_string())),
        "jne" | "jnz" => Ok(Inst::Jne(v[0].to_string())),
        "jl" | "jnge" => Ok(Inst::Jl(v[0].to_string())),
        "jle" | "jng" => Ok(Inst::Jle(v[0].to_string())),
        "jg" | "jnle" => Ok(Inst::Jg(v[0].to_string())),
        "jge" | "jnl" => Ok(Inst::Jge(v[0].to_string())),
        "nop" => Ok(Inst::Nop),
        "int" => Ok(Inst::Int(v[0].parse().map_err(|_| "bad int")?)),
    ...
    pub fn assemble(program: &Program) -> Result<Vec<u8>, String> {
        ...
        for block in &program.blocks {
            ...
            for line in &block.lines {
                ...
                if let Some(target) = t.strip_prefix("emit ") {
                ...
                if let Some(cond_str) = t.strip_prefix("if ") {
                    let reg = match cond_str.trim().parse::<i32>() {
                        ...
                        Err(_) => parse_i386_reg(cond_str.trim()).ok_or_else(|| {
                            format!("unknown register '{}' for if", cond_str.trim())
                        })?,
                    };
                ...
                if let Some(cond_str) = t.strip_prefix("elif ") {
                    ...
                    let reg = match cond_str.trim().parse::<i32>() {
                        ...
                        Err(_) => parse_i386_reg(cond_str.trim()).ok_or_else(|| {
                            format!("unknown register '{}' for elif", cond_str.trim())
                        })?,
                    };
                    items.push(Inst::CmpRegImm(reg, 0));
                    let beqz_idx = items.len();
                ...
                if let Some(cond_str) = t.strip_prefix("while ") {
                    let reg = match cond_str.trim().parse::<i32>() {
                        ...
                        Err(_) => parse_i386_reg(cond_str.trim()).ok_or_else(|| {
                            format!("unknown register '{}' for while", cond_str.trim())
                        })?,
                    };
                ...
                match lower(t) {
                    Ok(i) => items.push(i),
                    Err(e) => return Err(format!("line '{t}': {e}")),
                }
            }
        }
        ...
        // Peephole
        {
            ...
            while i < items.len() {
                ...
                if i + 1 < items.len() && is_nop(&items[i]) && is_nop(&items[i + 1]) {
                    items.remove(i + 1);
                    continue;
                }
                if is_term(&items[i]) {
                    let mut j = i + 1;
                    while j < items.len() && !is_label(&items[j]) {
                        j += 1;
                    }
                    if j > i + 1 {
                        items.drain(i + 1..j);
        ...
        for d in &program.data {
            match d {
                DataDecl::String { name, value } => {
                    ...
                    items.push(Inst::Bytes(expand_str(value)));
                }
        ...
        let labels = layout(&items)?;
        encode_items(&items, &labels, 0)
    }
}
...
fn layout(items: &[Inst]) -> Result<BTreeMap<String, u32>, String> {
    ...
    for item in items {
        match item {
            Inst::Label(n) => {
                if m.insert(n.clone(), o).is_some() {
                    return Err(format!("dup '{n}'"));
                }
            }
...
fn encode_items(
    ...
    let mut bin = Vec::new();
    for item in items {
        if let Inst::Label(_) = item {
        } else if let Inst::Bytes(b) = item {
            bin.extend_from_slice(b);
        } else {
            bin.extend_from_slice(&encode_inst(item, bin.len() as usize, labels)?);
        }
    }
    Ok(bin)
}

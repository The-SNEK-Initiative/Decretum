...
fn ipack(insts: &[IInst]) -> Vec<u8> {
    ...
    for chunk in insts.chunks(3) {
        let s0 = if chunk.len() > 0 {
            ienc_op(&chunk[0])
        } else {
            0
        };
        let s1 = if chunk.len() > 1 {
            ienc_op(&chunk[1])
        } else {
            0
        };
        let s2 = if chunk.len() > 2 {
            ienc_op(&chunk[2])
        } else {
            0
        };
...
impl DirectIa64Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Ia64BuildOutput, String> {
        if p.target != "ia64" {
            return Err(format!("need 'ia64', got '{}'", p.target));
        }
        ...
        for b in &p.blocks {
            ...
            for l in &b.lines {
                ...
                match ilower(t) {
                    Ok(i) => insts.push(i),
                    Err(_) => return Err(format!("ia64: '{}'", t)),
                }
            }
        }

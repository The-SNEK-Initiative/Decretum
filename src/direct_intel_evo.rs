...
pub struct I4004BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct I4004Builder;
pub struct I8008BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct I8008Builder;
pub struct I8080BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct I8080Builder;
pub struct I8086BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
...
impl I4004Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<I4004BuildOutput, String> {
        if p.target != "i4004" {
            return Err(format!("need 'i4004', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    ...
                    "wr1" => vec![0xC1],
                    "wr2" => vec![0xC2],
                    "wr3" => vec![0xC3], // Write index
                    "rd0" => vec![0xC4],
                    "rd1" => vec![0xC5],
                    "rd2" => vec![0xC6],
                    "rd3" => vec![0xC7],                  // Read index
...
impl I8008Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<I8008BuildOutput, String> {
        if p.target != "i8008" {
            return Err(format!("need 'i8008', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if let Some(cond) = t.strip_prefix("elif ") {
                    ...
                    let last = frame.br_indices.pop().ok_or("no branch".to_string())?;
                    let here = bin.len();
                    bin[last] = (here & 0xFF) as u8;
                    bin[last + 1] = ((here >> 8) & 0xFF) as u8;
                    let bra = bin.len();
                    bin.push(0xC4);
                    bin.extend(&[0, 0]);
                    frame.br_indices.push(bra);
                ...
                if t == "endif" {
                    ...
                    let target = bin.len();
                    for &pos in &frame.br_indices {
                        bin[pos] = (target & 0xFF) as u8;
                        bin[pos + 1] = ((target >> 8) & 0xFF) as u8
                    }
                    continue;
                }
                if let Some(cond) = t.strip_prefix("while ") {
                ...
                if t == "endwhile" {
                    ...
                    bin.push(0xC4);
                    bin.push((frame.start_pos & 0xFF) as u8);
                    bin.push(((frame.start_pos >> 8) & 0xFF) as u8);
                    let target = bin.len();
                    for &pos in &frame.br_indices {
                        bin[pos] = (target & 0xFF) as u8;
                        bin[pos + 1] = ((target >> 8) & 0xFF) as u8
                ...
                bin.extend(match m {
                    ...
                    "cmp" => vec![0xB8 | ra(0)],
                    "cpi" => vec![0xFE, ap(0)],
                    "jmp" => vec![0xC4, ap(0), ap(1)],
                    "jz" => vec![0xCA, ap(0), ap(1)],
                    "jnz" => vec![0xC2, ap(0), ap(1)],
                    "call" => vec![0xCD, ap(0), ap(1)],
                    "ret" => vec![0xC9],
                    "inr" => vec![0x04 | (ra(0) << 3)],
                    "dcr" => vec![0x04 | (ra(0) << 3) | 0x08],
                    "in" => vec![0x40 | ap(0)],
                    "out" => vec![0x41 | ap(0)],
                    "nop" => vec![0x00],
...
impl I8080Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<I8080BuildOutput, String> {
        if p.target != "i8080" {
            return Err(format!("need 'i8080', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    ...
                    "ori" => vec![0xF6, ap(0)],
                    "xri" => vec![0xEE, ap(0)],
                    "push" => vec![0xC5 | (ra(0) << 4)],
                    "pop" => vec![0xC1 | (ra(0) << 4)],
                    "xthl" => vec![0xE3],
                    "sphl" => vec![0xF9],
                    "pchl" => vec![0xE9],
                    "sta" => vec![0x32, ap(0), ap(1)],
                    "lda" => vec![0x3A, ap(0), ap(1)],
                    "shld" => vec![0x22, ap(0), ap(1)],
                    "lhld" => vec![0x2A, ap(0), ap(1)],
                    "cma" => vec![0x2F],
                    "stc" => vec![0x37],
                    "cmc" => vec![0x3F],
                    "di" => vec![0xF3],
...
impl I8086Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<I8086BuildOutput, String> {
        if p.target != "i8086" {
            return Err(format!("need 'i8086', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t == "endif" {
                    ...
                    let target = bin.len();
                    for &pos in &frame.br_indices {
                        bin[pos] = target.wrapping_sub(pos + 1) as u8
                    }
                    continue;
                }
                if let Some(cond) = t.strip_prefix("while ") {
                ...
                bin.extend(match m {
                    "mov" if args.len() == 2 => {
                        if is_8r(args[0]) {
                            vec![
                                0xB0 | (ra(0) & 7) + (if wide(0) == 0 { 0 } else { 8 }),
                                ap(1),
                            ]
                        } else {
                            vec![0xB8 | ra(0), ap(1), ap(2)]
                        }
                    }

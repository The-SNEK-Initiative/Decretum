...
impl JovialBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<JovialBuildOutput, String> {
        if p.target != "jovial" {
            return Err(format!("need 'jovial'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                let rp = |i: usize| {
                    a.get(i)
                        .and_then(|s| s.trim_start_matches('r').parse::<u8>().ok())
                        .unwrap_or(0)
                };
                ...
                if t == "endif" {
                    ...
                    let end = bin.len();
                    if frame.cond_br > 0 {
                        bin[frame.cond_br] = end as u8
                    }
                    for &b in &frame.br_indices {
                        bin[b] = end as u8
                    }
                    continue;
                }
                ...
                bin.extend(match m {
                    "add" => vec![0x10, rp(0), rp(1)],
                    "sub" => vec![0x11, rp(0), rp(1)],
                    "mul" => vec![0x12, rp(0), rp(1)],
                    "div" => vec![0x13, rp(0), rp(1)],
                    "mov" => vec![0x01, rp(0), rp(1)],
                    "cmp" => vec![0x14, rp(0), rp(1)],
                    "jmp" => vec![0x20, ap(0)],
                    "jne" => vec![0x21, ap(0)],
                    "jeq" => vec![0x22, ap(0)],
                    "call" => vec![0x30, ap(0), ap(1)],
                    "ret" => vec![0x40],
                    "nop" => vec![0x00],

...
impl M16cBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<M16cBuildOutput, String> {
        if p.target != "m16c" {
            return Err(format!("need 'm16c'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    bin[prev + 1] = rel as u8;
                    bin[prev + 2] = (rel >> 8) as u8;
                    let jmp_idx = bin.len();
                    bin.extend(&[0xFA, 0, 0]);

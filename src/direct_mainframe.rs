...
pub struct S360BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct S360Builder;
pub struct ZArchBuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
...
fn menc_360(i: &MInst) -> Vec<u8> {
    match i.op {
        0x1A..=0x1F | 0xB2..=0xB5 | 0xB9..=0xBF | 0xE3 | 0xEB | 0xE5 => {
            // RR/RRE
            let mut v = vec![i.op];
            v.push(i.r1 << 4 | i.r2);
            v
        ...
        0x50..=0x5F | 0x40..=0x4F => {
            // RX
            let mut v = vec![i.op, i.r1 << 4 | i.x2, i.b1];
            v.extend(&i.d1.to_be_bytes());
            v
        }
        0x80..=0x8F | 0x90..=0x9F | 0xA0..=0xAF => {
            // RS/SI
            let mut v = vec![i.op, i.r1 << 4 | i.r2, i.b1];
            v.extend(&i.d1.to_be_bytes());
            v
        }
        _ => {
            // SS or default
            let mut v = vec![i.op, i.r1 << 4 | i.r2, i.b1];
            v.extend(&i.d1.to_be_bytes()[..2]);
            v.push(i.b2);
            v.extend(&i.d2.to_be_bytes()[..2]);
            v
        }
    }
}
fn menc_zarch(i: &MInst) -> Vec<u8> {
    let mut base = menc_360(i);
    if i.is_z && i.op >= 0xE0 {
        base.extend(&40u32.to_be_bytes()[..2]);
    }
    base
}
...
fn mlower(t: &str, is_z: bool) -> Result<MInst, String> {
    ...
    Ok(match m {
        ...
        "sll" if args.len() == 2 => MInst {
            op: 0x89,
            op2: 0,
            r1: mrn(gr(args[0])?),
            r2: args[1].parse::<u8>().unwrap_or(0),
            x2: 0,
            b1: 0,
            b2: 0,
            d1: 0,
    ...
    for b in &p.blocks {
        for l in &b.lines {
            ...
            if let Some(r) = t.strip_prefix("if ") {
                ...
                let p = bin.len();
                bin.extend(&[0x47, 0x80, 0x00, 0x00, 0x00]); // BC 8, d1
                cf_stack.push(CfFrame {
                    kind: CfKind::If,
                    endif_label: format!("_cf{}", cf_counter),
                    else_label: format!("_cf{}_else", cf_counter),
                    br_indices: vec![p],
                    has_else: false,
                    start_pos: 0,
                });
            ...
            if let Some(r) = t.strip_prefix("elif ") {
                ...
                bin[prev + 4] = ba[1];
                let jmp_idx = bin.len();
                bin.extend(&[0x47, 0xF0, 0x00, 0x00, 0x00]);
                f.br_indices.push(jmp_idx);
                let rn = r.trim().trim_start_matches('r').parse::<u8>().unwrap_or(0);
                bin.extend(&[0x12, (rn << 4) | rn]);
                let p = bin.len();
                bin.extend(&[0x47, 0x80, 0x00, 0x00, 0x00]);
                f.br_indices.push(p);
                continue;
            }
            if t == "else" {
            ...
            if t == "endif" {
                ...
                if !matches!(f.kind, CfKind::If) {
                    return Err("endif without matching if".into());
                }
                let addr = bin.len() as u16;
            ...
            if t == "endwhile" {
                ...
                if !matches!(f.kind, CfKind::While) {
                    return Err("endwhile without matching while".into());
                }
            ...
            if let Ok(i) = mlower(t, is_z) {
                bin.extend(if is_z { menc_zarch(&i) } else { menc_360(&i) });
            }
        }
    }
...
impl S360Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<S360BuildOutput, String> {
        if p.target != "s360" {
            return Err(format!("need 's360', got '{}'", p.target));
        }
        let bin = mbuild(p, false)?;
...
impl ZArchBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<ZArchBuildOutput, String> {
        if p.target != "zarch" {
            return Err(format!("need 'zarch', got '{}'", p.target));
        }
        let bin = mbuild(p, true)?;
        std::fs::write(out, &bin).map_err(|e| e.to_string())?;
        Ok(ZArchBuildOutput {
            bin_path: out.to_path_buf(),
            bin_size: bin.len(),

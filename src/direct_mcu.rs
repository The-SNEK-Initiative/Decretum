...
fn mci2(v: u16) -> Vec<u8> {
    v.to_le_bytes().to_vec()
}
fn mci4(v: u32) -> Vec<u8> {
    v.to_le_bytes().to_vec()
}

fn mcenc(i: &MCInst, arch: &str) -> Result<Vec<u8>, String> {
    Ok(match i {
        MCInst::Label(_) => vec![],
        MCInst::Bytes(b) => b.clone(),
...
fn mcparse(t: &str, arch: &str) -> Result<MCInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(MCInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        "nop" => match arch {
            "pic" => Ok(MCInst::Op2(0x0000)),
            "avr" => Ok(MCInst::Op2(0x0000)),
            _ => Err("bad".into()),
        },
        "ret" => Ok(MCInst::Ret),
        "mov" | "movwf" if v.len() == 2 => {
            let d: u8 = v[1]
        ...
        "add" | "addwf" if v.len() == 2 => {
            let f: u16 = v[1].trim_start_matches('f').parse().unwrap_or(0);
            match arch {
                "pic" => Ok(MCInst::Op2(0x0700 | (f & 0x7F))),
                "avr" => Ok(MCInst::Op2(0x0C00 | (f & 0xF))),
                _ => Err("bad".into()),
            }
        }
        "sub" | "subwf" if v.len() == 2 => {
            let f: u16 = v[1].trim_start_matches('f').parse().unwrap_or(0);
            match arch {
                "pic" => Ok(MCInst::Op2(0x0800 | (f & 0x7F))),
                "avr" => Ok(MCInst::Op2(0x1800 | (f & 0xF))),
                _ => Err("bad".into()),
            }
        }
        "bra" | "goto" if v.len() == 1 => match arch {
            "pic" => Ok(MCInst::Op2(0x2800)),
            "avr" => Ok(MCInst::Op2(0xC000)),
            _ => Err("bad".into()),
        },
        "bne" if v.len() == 1 => match arch {
            "avr" => Ok(MCInst::Op2(0xF401)),
            _ => Err("bad".into()),
        },
        "beq" if v.len() == 1 => match arch {
            "avr" => Ok(MCInst::Op2(0xF001)),
            _ => Err("bad".into()),
        },
...
fn mc(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if t.ends_with(':') {
                items.push(MCInst::Label(format!(
                    "{}.{}",
                    b.name,
                    t[..t.len() - 1].trim()
                )));
            ...
            if t == "ret" || t.starts_with("emit") || t.starts_with("call") {
                items.push(MCInst::Ret);
                continue;
            }
    ...
    crate::direct_peephole::peephole(
        ...
        |i| {
            matches!(i, MCInst::Op2(0x2800 | 0xC000))
                || matches!(i, MCInst::CondBranch(..))
                || matches!(i, MCInst::UncondBranch(_))
        },
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                ...
                b.push(0);
                items.push(MCInst::Label(format!("__data_{name}")));
                items.push(MCInst::Bytes(b));
            }
            DataDecl::Scalar { name, width, value } => {
                items.push(MCInst::Label(format!("__data_{name}")));
                items.push(MCInst::Bytes(match width {
            ...
            DataDecl::Buffer { name, size } => {
                items.push(MCInst::Label(format!("__data_{name}")));
                items.push(MCInst::Bytes(vec![0u8; *size]));
            }
        }
    }

...
pub struct Mil1750aBuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct Mil1750aBuilder;

impl Mil1750aBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Mil1750aBuildOutput, String> {
        if p.target != "mil1750a" {
            return Err(format!("need 'mil1750a'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0x0C, 0xE0]);
                    continue;
                }
                if let Some(r) = t.strip_prefix("if ") {
                    let rn = r.trim().trim_start_matches('r').parse::<u8>().unwrap_or(0);
                    cf_counter += 1;
                    let cmp = 0x6200u16 | ((rn as u16) << 8);
                    bin.extend(&cmp.to_be_bytes());
                    let p = bin.len();
                    bin.extend(&[0x06, 0x00]);
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    let nw = (ow & !0x01FF) | (rel as u16 & 0x01FF);
                    let nb = nw.to_be_bytes();
                    bin[prev] = nb[0];
                    bin[prev + 1] = nb[1];
                    let jmp_idx = bin.len();
                    bin.extend(&[0x02, 0x00]);
                ...
                let a: Vec<&str> = j
                    ...
                    .collect();
                let rp = |i: usize| {
                    let s = *a.get(i).unwrap_or(&"");
                    s.trim_start_matches('r').parse::<u8>().unwrap_or(0)
                };
                ...
                bin.extend(match m {
                    ...
                    "jmp" => w(0x0200u16 | (ap(0) & 0x01FF)),
                    "jsr" => w(0x7E00u16 | (ap(0) & 0x01FF)),
                    "bne" => w(0x0800u16 | (ap(0) & 0x01FF)),
                    "beq" => w(0x0600u16 | (ap(0) & 0x01FF)),
                    "ret" => w(0x0CE0),
                    "nop" => w(0),

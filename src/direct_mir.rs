...
impl MirBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<MirBuildOutput, String> {
        if p.target != "mir" {
            return Err(format!("need 'mir'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    ...
                    "pop" => w(0x06000000u32),
                    "jmp" => w(0x07000000u32 | (ap(0) & 0x00FFFFFF)),

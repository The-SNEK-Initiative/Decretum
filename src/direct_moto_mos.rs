...
pub struct M6800BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
pub struct M6800Builder;
pub struct Mos6501BuildOutput {
    pub bin_path: PathBuf,
    pub bin_size: usize,
}
...
impl M6800Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<M6800BuildOutput, String> {
        if p.target != "m6800" {
            return Err(format!("need 'm6800', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if let Some(r) = t.strip_prefix("elif ") {
                    ...
                    let rel = (bin.len() as i16 - prev as i16 - 2) as i8 as u8;
                    bin[prev + 1] = rel;
                    let jmp_idx = bin.len();
                    bin.extend(&[0x20, 0]);
                ...
                if t == "endif" {
                    ...
                    for &idx in &f.br_indices {
                        let rel = (bin.len() as i16 - idx as i16 - 2) as i8 as u8;
                        bin[idx + 1] = rel;
                    }
                    continue;
                }
                ...
                bin.extend(match m {
                    ...
                    "staa" | "sta" => vec![0x97, ap(0)],
                    "stab" | "stb" => vec![0xD7, ap(0)],
...
impl Mos6501Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Mos6501BuildOutput, String> {
        if p.target != "mos6501" {
            return Err(format!("need 'mos6501', got '{}'", p.target));
        }
        let mut bin = Vec::new();
        struct CfFrame {
            kind: CfKind,

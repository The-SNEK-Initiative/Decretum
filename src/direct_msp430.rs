...
impl Msp430Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Msp430BuildOutput, String> {
        if p.target != "msp430" {
            return Err(format!("need 'msp430'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x12, 0xB0, 0, 0]);
                    continue;
                }
                if t == "ret" || t == "hlt" {
                    bin.extend(&[0x41, 0x30]);
                    continue;
                }
                let parts: Vec<&str> = t
                    .split(|c: char| c == ' ' || c == '\t')
                ...
                bin.extend(match m {
                    ...
                    "inc" => w(0x5300u16 | ((rp(0) & 0xF) << 8)),
                    "dec" => w(0x7300u16 | ((rp(0) & 0xF) << 8)),
                    "jmp" => vec![0x30, 0],
                    "jne|jnz" => vec![0x20, 0],
                    "jeq|jz" => vec![0x24, 0],
                    "call" => vec![0x12, 0xB0, 0, 0],
                    "ret" => vec![0x41, 0x30],
                    "rra" => vec![0x11, 0],
                    "rrc" => vec![0x10, 0],
                    "push" => vec![0x12, 0],

...
impl Nec78kBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Nec78kBuildOutput, String> {
        if p.target != "nec78k" {
            return Err(format!("need 'nec78k'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    "mov" => vec![0x40u8 | ap(0), 0],
                    "xch" => vec![0x60],
                    "add" => vec![0x20u8 | ap(0), 0],
                    "sub" => vec![0x30u8 | ap(0), 0],

...
pub struct Quantum8BuildOutput {
...
pub fn qparse(p: &Program, max_qubits: u8) -> Result<Vec<QOp>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            let qp = |s: &str| -> Result<u8, String> {
                ...
                if q >= max_qubits {
                    return Err(format!(
                        "qubit {} out of range for this target (max {})",
                        q,
                        max_qubits - 1
                    ));
                }
            ...
            let op = match m {
                "h" | "hadamard" if args.len() == 1 => Some(QOp {
                    gate: Gate::H,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "x" if args.len() == 1 => Some(QOp {
                    gate: Gate::X,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "y" if args.len() == 1 => Some(QOp {
                    gate: Gate::Y,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "z" if args.len() == 1 => Some(QOp {
                    gate: Gate::Z,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "cx" | "cnot" if args.len() == 2 => Some(QOp {
                    gate: Gate::CX,
                    qubits: vec![qp(args[0])?, qp(args[1])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "ccx" | "toffoli" if args.len() == 3 => Some(QOp {
                    gate: Gate::CCX,
                    qubits: vec![qp(args[0])?, qp(args[1])?, qp(args[2])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "swap" if args.len() == 2 => Some(QOp {
                    gate: Gate::Swap,
                    qubits: vec![qp(args[0])?, qp(args[1])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "s" if args.len() == 1 => Some(QOp {
                    gate: Gate::S,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "t" if args.len() == 1 => Some(QOp {
                    gate: Gate::T,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "sdg" | "sdag" if args.len() == 1 => Some(QOp {
                    gate: Gate::Sdg,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "tdg" | "tdag" if args.len() == 1 => Some(QOp {
                    gate: Gate::Tdg,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
                }),
                "rx" if args.len() == 2 => Some(QOp {
                    gate: Gate::Rx(ang(args[1])?),
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: ang(args[1])? * PI / 180.0,
                }),
                "ry" if args.len() == 2 => Some(QOp {
                    gate: Gate::Ry(ang(args[1])?),
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: ang(args[1])? * PI / 180.0,
                }),
                "rz" if args.len() == 2 => Some(QOp {
                    gate: Gate::Rz(ang(args[1])?),
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: ang(args[1])? * PI / 180.0,
                }),
                "measure" if args.len() == 2 => Some(QOp {
                    gate: Gate::Measure,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![cp(args[1])?],
                    angle: 0.0,
                }),
                "reset" if args.len() == 1 => Some(QOp {
                    gate: Gate::Reset,
                    qubits: vec![qp(args[0])?],
                    cbits: vec![],
                    angle: 0.0,
    ...
    if !entry_found {
        return Err(format!("entry event '{}' not found", p.entry_event));
    }
    Ok(ops)
}

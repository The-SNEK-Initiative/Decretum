...
pub fn merge_adjacent_rotations(ops: &mut Vec<QOp>) {
    ...
    while i + 1 < ops.len() {
        ...
        if let Some(g) = merged {
            ops[i].gate = g;
            ops[i].angle = match g {
                Gate::Rx(a) | Gate::Ry(a) | Gate::Rz(a) => a,
                _ => 0.0,
            };
            ops.remove(i + 1);
            continue;
...
pub fn remove_unused_measurements(ops: &mut Vec<QOp>) {
    ...
    ops.retain(|op| {
        ...
        op.cbits
            .iter()
            .any(|&c| (c as usize) < MAX_CBITS && cbits_used[c as usize])
    });
}
...
pub fn from_qasm(source: &str) -> Result<(Vec<QOp>, usize, usize), String> {
    ...
    for line in source.lines() {
        ...
        let op = match parts[0] {
            ...
            "cx" => Some(QOp {
                gate: Gate::CX,
                qubits: vec![parse_q(parts[1]), parse_q(parts[2])],
                cbits: vec![],
                angle: 0.0,
            }),
            "ccx" => Some(QOp {
                gate: Gate::CCX,
                qubits: vec![parse_q(parts[1]), parse_q(parts[2]), parse_q(parts[3])],
                cbits: vec![],
                angle: 0.0,
            }),
            "swap" => Some(QOp {
            ...
            "rx" => Some(QOp {
                gate: Gate::Rx(0.0),
                qubits: vec![parse_q(parts[2])],
                cbits: vec![],
                angle: parts[1].parse::<f64>().unwrap_or(0.0),
            }),
            "ry" => Some(QOp {
                gate: Gate::Ry(0.0),
                qubits: vec![parse_q(parts[2])],

...
enum RInstr {
    Label(String),
    Bytes(Vec<u8>),
    Rtype(RReg, RReg, RReg, u32, u32, u32),
    Itype(RReg, RReg, u32, u32),
    Utype(RReg, u32, u32),
    Jtype(u32, u32),
    Branch(String, u32, RReg, RReg),
    Ret,
}
...
fn rparse(t: &str, arch: &str) -> Result<RInstr, String> {
    ...
    if t.ends_with(':') {
        return Ok(RInstr::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        ...
        "add" | "sub" | "and" | "or" | "xor" if v.len() == 3 => {
            ...
            let (op, funct, subf) = match (m, arch) {
                ("add", "mips") => (0u32, 0x20, 0),
                ("sub", "mips") => (0u32, 0x22, 0),
                ("and", "mips") => (0u32, 0x24, 0),
                ("or", "mips") => (0u32, 0x25, 0),
                ("xor", "mips") => (0u32, 0x26, 0),
                ("add", "ppc") => (0x1Cu32 << 26, 0, 0x214),
                ("sub", "ppc") => (0x1Cu32 << 26, 0, 0x294),
                ("and", "ppc") => (0x1Cu32 << 26, 0, 0x01C),
                ("or", "ppc") => (0x1Cu32 << 26, 0, 0x21C),
                ("xor", "ppc") => (0x1Cu32 << 26, 0, 0x29C),
                ("add", "sparc") => (0, 0, 0),
                ("sub", "sparc") => (0x04u32 << 19, 0, 0),
                ("and", "sparc") => (0x05u32 << 19, 0, 0),
                ("or", "sparc") => (0x06u32 << 19, 0, 0),
                ("xor", "sparc") => (0x07u32 << 19, 0, 0),
...
fn optimise_mips(items: &mut Vec<RInstr>) {
    ...
    let is_jump_or_branch =
        |i: &RInstr| matches!(i, RInstr::Branch(..) | RInstr::Jtype(..) | RInstr::Ret);
    ...
    // DEV: If the slot holds a NOP, move the preceding non-branch, non label instruction into it.
    {
        let mut i = 0;
        while i + 2 < items.len() {
            let at_branch = matches!(
                items[i],
                RInstr::Branch(..) | RInstr::Jtype(..) | RInstr::Ret
            );
            if at_branch
                && is_nop(&items[i + 1])
                && i > 0
                && !matches!(items[i - 1], RInstr::Label(_))
                && !is_jump_or_branch(&items[i - 1])
            {
...
fn optimise_ppc(items: &mut Vec<RInstr>) {
    ...
    while i + 1 < items.len() {
        if matches!(&items[i], RInstr::Rtype(.., 0x4C000000, 0x21, _)) &&  // crand = op=0x13<<26 -> 0x4C000000
           matches!(&items[i+1], RInstr::Rtype(.., 0x4C000000, 0x21, _))
        { // consecutive crand
        }
        i += 1;
    }
        ...
        while i < items.len() {
            if has_op3(&items[i], 0x1C) {
                ...
                while j < items.len() && depth > 0 {
                    if has_op3(&items[j], 0x1C) {
                        depth += 1;
                    } else if has_op3(&items[j], 0x1D) {
                        depth -= 1;
                    }
                    j += 1;
                }
...
fn rarch(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if t == "ret" {
                items.push(RInstr::Ret);
                continue;
            }

            // if <reg>
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = rrp(cond_str.trim())
                ...
                match arch {
                    "mips" => {
                        items.push(RInstr::Branch(endif_lbl.clone(), 0x10000000, reg, RReg::R0))
                    }
                    "ppc" => {
                        items.push(RInstr::Branch(endif_lbl.clone(), 0x10000000, reg, RReg::R0))
                    }
                    _ => {
                        return Err(
                            "if/elif/endif/while/endwhile not supported for this arch".into()
                        );
                    }
                }
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let RInstr::Branch(ref mut l, ..) = items[prev] {
                    *l = elif_lbl.clone();
                }
                items.push(RInstr::Branch(
                    frame.endif_label.clone(),
                    0x10000000,
                    RReg::R0,
                    RReg::R0,
                ));
                ...
                let br_idx = items.len();
                match arch {
                    "mips" => items.push(RInstr::Branch(
                        frame.endif_label.clone(),
                        0x10000000,
                        reg,
                        RReg::R0,
                    )),
                    "ppc" => items.push(RInstr::Branch(
                        frame.endif_label.clone(),
                        0x10000000,
                        reg,
                        RReg::R0,

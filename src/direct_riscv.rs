...
impl DirectRiscvBuilder {
    pub fn build_bin(program: &Program, out_path: &Path) -> Result<RiscvBuildOutput, String> {
        if program.target != "riscv"
            && program.target != "riscv64"
            && program.target != "riscv_cheri"
        {
            return Err(format!(
...
fn encode_inst(
    ...
    let word = match inst {
        // R-type (opcode=0x33)
        Inst::Add(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x0, reg_num(*rd), 0x33)
        }
        Inst::Sub(rd, rs1, rs2) => {
            encode_r_type(0x20, reg_num(*rs2), reg_num(*rs1), 0x0, reg_num(*rd), 0x33)
        }
        Inst::Sll(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x1, reg_num(*rd), 0x33)
        }
        Inst::Slt(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x2, reg_num(*rd), 0x33)
        }
        Inst::Sltu(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x3, reg_num(*rd), 0x33)
        }
        Inst::Xor(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x4, reg_num(*rd), 0x33)
        }
        Inst::Srl(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x5, reg_num(*rd), 0x33)
        }
        Inst::Sra(rd, rs1, rs2) => {
            encode_r_type(0x20, reg_num(*rs2), reg_num(*rs1), 0x5, reg_num(*rd), 0x33)
        }
        Inst::Or(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x6, reg_num(*rd), 0x33)
        }
        Inst::And(rd, rs1, rs2) => {
            encode_r_type(0x00, reg_num(*rs2), reg_num(*rs1), 0x7, reg_num(*rd), 0x33)
        }
        // RV32M extension (funct7=0x01)
        Inst::Mul(rd, rs1, rs2) => {
            encode_r_type(0x01, reg_num(*rs2), reg_num(*rs1), 0x0, reg_num(*rd), 0x33)
        }
        Inst::Div(rd, rs1, rs2) => {
            encode_r_type(0x01, reg_num(*rs2), reg_num(*rs1), 0x4, reg_num(*rd), 0x33)
        }
        Inst::Rem(rd, rs1, rs2) => {
            encode_r_type(0x01, reg_num(*rs2), reg_num(*rs1), 0x6, reg_num(*rd), 0x33)
        }
        // I-type arithmetic (opcode=0x13)
        Inst::Addi(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x0, reg_num(*rd), 0x13)
        }
        Inst::Slti(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x2, reg_num(*rd), 0x13)
        }
        Inst::Sltiu(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x3, reg_num(*rd), 0x13)
        }
        Inst::Xori(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x4, reg_num(*rd), 0x13)
        }
        Inst::Ori(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x6, reg_num(*rd), 0x13)
        }
        Inst::Andi(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x7, reg_num(*rd), 0x13)
        }
        ...
        Inst::Srai(rd, rs1, shamt) => {
            encode_i_type(*shamt | 0x400, reg_num(*rs1), 0x5, reg_num(*rd), 0x13)
        }
        // Load (opcode=0x03)
        Inst::Lb(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x0, reg_num(*rd), 0x03)
        }
        Inst::Lh(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x1, reg_num(*rd), 0x03)
        }
        Inst::Lw(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x2, reg_num(*rd), 0x03)
        }
        Inst::Lbu(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x4, reg_num(*rd), 0x03)
        }
        Inst::Lhu(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x5, reg_num(*rd), 0x03)
        }
        // Store (opcode=0x23)
        Inst::Sb(rs2, rs1, imm) => {
            encode_s_type(imm.0 as u32, reg_num(*rs2), reg_num(*rs1), 0x0, 0x23)
        }
        Inst::Sh(rs2, rs1, imm) => {
            encode_s_type(imm.0 as u32, reg_num(*rs2), reg_num(*rs1), 0x1, 0x23)
        }
        Inst::Sw(rs2, rs1, imm) => {
            encode_s_type(imm.0 as u32, reg_num(*rs2), reg_num(*rs1), 0x2, 0x23)
        }
        // U-type
        ...
        Inst::Jalr(rd, rs1, imm) => {
            encode_i_type(imm.0 as u32, reg_num(*rs1), 0x0, reg_num(*rd), 0x67)
        }
        // Special
        ...
        Inst::Li(rd, val) => {
            if *val >= -2048 && *val <= 2047 {
                ...
                let _upper =
                    ((*val >> 12) + if (*val & 0x800) != 0 { 1 } else { 0 }) as u32 & 0xFFFFF;
...
fn lower_line(line: &str, _data_labels: &BTreeMap<String, Vec<u8>>) -> Result<Inst, String> {
    ...
    match mnemonic {
        ...
        "beqz" => {
            ...
            if args.len() == 2 {
                let rs =
                    parse_reg(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
        ...
        "jalr" => {
            ...
            if args.len() == 3 {
                let rd =
                    parse_reg(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
                let rs1 =
                    parse_reg(args[1]).ok_or_else(|| format!("unknown register '{}'", args[1]))?;
                let imm =
                    parse_imm12(args[2]).ok_or_else(|| format!("bad immediate '{}'", args[2]))?;
        ...
        "li" | "mv" | "neg" | "not" | "seqz" | "snez" => {
            ...
            if mnemonic == "li" && args.len() == 2 {
                ...
                let val: i64 = args[1]
                    .parse()
                    .map_err(|_| format!("bad immediate '{}'", args[1]))?;
                Ok(Inst::Li(rd, val))
            } else if (mnemonic == "mv"
                || mnemonic == "neg"
                || mnemonic == "not"
...
fn parse_two_reg_imm(s: &str) -> Result<(Reg, Reg, Imm12), String> {
    ...
    if args.len() != 3 {
        return Err(format!(
            "expected 2 registers + immediate, got {}",
            args.len()
        ));
    }
    let r1 = parse_reg(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
...
fn parse_two_reg_shift(s: &str) -> Result<(Reg, Reg, u32), String> {
    ...
    if args.len() != 3 {
        return Err(format!(
            "expected 2 registers + shift amount, got {}",
            args.len()
        ));
    }
    let r1 = parse_reg(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
...
fn parse_load_store(
    mnemonic: &str,
    s: &str,
    f: impl FnOnce(Reg, Reg, Imm12) -> Inst,
) -> Result<Inst, String> {
    let args: Vec<&str> = s
        .split(',')
        .map(|s| s.trim())
    ...
    if let Some(rest) = mem.strip_suffix(')') {
        if let Some(idx) = rest.find('(') {
            ...
            let rs1 =
                parse_reg(reg_str).ok_or_else(|| format!("unknown register '{}'", reg_str))?;
...
impl DirectRiscvAssembler {
    fn assemble(program: &Program) -> Result<Vec<u8>, String> {
            ...
            for line in &block.lines {
                ...
                if let Some(cond_str) = trimmed.strip_prefix("if ") {
                    let reg = match cond_str.trim().parse::<i64>() {
                        ...
                        Err(_) => {
                            ...
                            parse_reg(reg_str)
                                .ok_or_else(|| format!("unknown register '{}' for if", reg_str))?
                        }
                    };
                ...
                if let Some(cond_str) = trimmed.strip_prefix("elif ") {
                    ...
                    let prev = frame
                        .beqz_indices
                        .last()
                        .ok_or("internal: no beqz indices")?;
                    ...
                    let reg = match cond_str.trim().parse::<i64>() {
                        ...
                        Err(_) => {
                            let reg_str = cond_str.trim();
                            parse_reg(reg_str)
                                .ok_or_else(|| format!("unknown register '{}' for elif", reg_str))?
                        }
                    };
                    let beqz_idx = items.len();
                ...
                if let Some(cond_str) = trimmed.strip_prefix("while ") {
                    let reg = match cond_str.trim().parse::<i64>() {
                        ...
                        Err(_) => {
                            let reg_str = cond_str.trim();
                            parse_reg(reg_str).ok_or_else(|| {
                                format!("unknown register '{}' for while", reg_str)
                            })?
                        }
                    };
        ...
        crate::direct_peephole::peephole(
            &mut items,

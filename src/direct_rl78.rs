...
impl Rl78Builder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<Rl78BuildOutput, String> {
        if p.target != "rl78" {
            return Err(format!("need 'rl78'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                bin.extend(match m {
                    "mov" => vec![0x04, ap(0), ap(1)],
                    "xch" => vec![0x0C, ap(0), ap(1)],
                    "add" => vec![0x08, ap(0), ap(1)],
                    "sub" => vec![0x18, ap(0), ap(1)],

...
impl RxBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<RxBuildOutput, String> {
        if p.target != "rx" {
            return Err(format!("need 'rx'"));
        }
        ...
        for b in &p.blocks {
            for l in &b.lines {
                ...
                if t.starts_with("emit ") || t.starts_with("call ") {
                    bin.extend(&[0x08, 0, 0, 0]);
                    continue;
                }
                if t == "ret" {
                    bin.extend(&[0x0C, 0, 0, 0]);
                    continue;
                }
                if t == "hlt" {
                ...
                if t.starts_with("if ") {
                    ...
                    bin.extend((0x50000000u32 | (reg as u32) << 4).to_le_bytes());
                    let pos = bin.len();
                    bin.extend(&[0, 0, 0, 0]);
                ...
                if t.starts_with("elif ") {
                    ...
                    let enc = 0x61000000u32 | (here & 0xFFFFFF);
                    bin[last..last + 4].copy_from_slice(&enc.to_le_bytes());
                    let bra_pos = bin.len();
                    bin.extend(&[0, 0, 0, 0]);
                ...
                if t == "endif" {
                    ...
                    for &pos in &frame.br_indices {
                        let enc = 0x62000000u32 | (target & 0xFFFFFF);
                        bin[pos..pos + 4].copy_from_slice(&enc.to_le_bytes())
                    }
                    continue;
                }
                ...
                if t == "endwhile" {
                    ...
                    bin.extend((0x62000000u32 | (back & 0xFFFFFF)).to_le_bytes());
                    let here = bin.len() as u32;
                    let enc = 0x61000000u32 | (here & 0xFFFFFF);
                    for &pos in &frame.br_indices {
                        bin[pos..pos + 4].copy_from_slice(&enc.to_le_bytes())

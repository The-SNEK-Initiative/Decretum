    ...
    ($B:ident, $O:ident, $T:expr) => {
        ...
        impl $B {
            pub fn build_bin(p: &Program, out: &Path) -> Result<$O, String> {
                let ok = if stringify!($B) == "DirectSh2Builder" {
                    p.target == "sh2" || p.target == "sh4"
                } else {
                    p.target == $T
                };
                if !ok {
                    return Err(format!("need '{}', got '{}'", $T, p.target));
                }
                let k = sharch(p, $T)?;
...
enum SInst {
    ...
    Su16(u16),
    Ret,
    CondBranch(u8, String),
    UncondBranch(String),
}

fn senc(i: &SInst, arch: &str) -> Result<Vec<u8>, String> {
    Ok(match i {
        SInst::Label(_) => vec![],
        SInst::Bytes(b) => b.clone(),
...
fn shparse(t: &str, arch: &str) -> Result<SInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(SInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    let gim = |s: &str| {
        ...
        s2.parse::<u8>().map_err(|_| "bad imm".to_string())
    };
    match m {
        "nop" => {
            let v: u16 = match arch {
        ...
        "mov" if v.len() == 2 => {
            ...
            match arch {
                "sh2" | "sh4" => {
                    if let Some(val) = imm {
                        let d: u8 = v[0].trim_start_matches('r').parse().unwrap_or(0);
                        Ok(SInst::Su16(0xE000 | ((d as u16) << 8) | val as u16))
                    } else {
                        Ok(SInst::Su16(0x6000))
                    } // MOV Rm,Rn
                }
                "m68k" => {
                    if let Some(val) = imm {
                        let d = v[0].trim_start_matches('d').parse::<u8>().unwrap_or(0);
                        Ok(SInst::Su16(0x303C | ((d as u16) << 9) | val as u16)) // MOVEQ
                    } else {
                        Ok(SInst::Su16(0x2000))
                    }
                }
                _ => Err("bad".into()),
            }
            ...
            match arch {
                "sh2" | "sh4" => {
                    if let Ok(val) = imm {
                        let d: u8 = v[0].trim_start_matches('r').parse().unwrap_or(0);
                        Ok(SInst::Su16(
                            (if m == "add" { 0x7000 } else { 0x8000 })
                                | ((d as u16) << 8)
                                | val as u16,
                        ))
                    } else {
                        Ok(SInst::Su16(if m == "add" { 0x300C } else { 0x3008 }))
                    }
                }
                "m68k" => {
                    let d: u8 = v[0].trim_start_matches('d').parse().unwrap_or(0);
                    Ok(SInst::Su16(if m == "add" {
                        0xD000 | (d as u16)
                    } else {
                        0x9000 | (d as u16)
                    }))
        ...
        "bra" if v.len() == 1 => {
            match arch {
                "sh2" | "sh4" => Ok(SInst::Su16(0xA000)), // BRA disp12
                "m68k" => Ok(SInst::Su16(0x6000)),
                _ => Err("bad".into()),
            }
        }
        "bne" | "beq" if v.len() == 1 => {
            let v: u16 = match (m, arch) {
                ("beq", "sh2") | ("beq", "sh4") => 0x8F00,
                ("bne", "sh2") | ("bne", "sh4") => 0x8E00,
                ("beq", "m68k") => 0x6700,
                ("bne", "m68k") => 0x6600,
                _ => 0x0000,
            };
...
fn sharch(p: &Program, arch: &str) -> Result<Vec<u8>, String> {
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(SInst::Label(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(SInst::Label(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" || t == "rts" {
                items.push(SInst::Ret);
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if matches!(frame.kind, CfKind::While) {
                    return Err("elif in while".to_string());
                }
                if frame.has_else {
            ...
            if t == "else" {
                ...
                if matches!(frame.kind, CfKind::While) {
                    return Err("else in while".to_string());
                }
                if frame.has_else {
            ...
            if t == "endif" {
                ...
                if matches!(frame.kind, CfKind::While) {
                    return Err("endif while expecting endwhile".to_string());
                }
    ...
    crate::direct_peephole::peephole(
        ...
        |i| {
            matches!(i, SInst::Su16(0xA000 | 0x6000))
                || matches!(i, SInst::CondBranch(..))
                || matches!(i, SInst::UncondBranch(_))
        },
    ...
    for d in &p.data {
        match d {
            DataDecl::String { name, value } => {
                let mut b = crate::direct_arch::expand_str(value);
                b.push(0);
                items.push(SInst::Label(format!("__data_{name}")));
                items.push(SInst::Bytes(b));
            }
            DataDecl::Scalar { name, width, value } => {
                items.push(SInst::Label(format!("__data_{name}")));
                items.push(SInst::Bytes(match width {
                    ScalarWidth::Byte => vec![*value as u8],
                    ScalarWidth::Word => (*value as u16).to_be_bytes().to_vec(),
                    ScalarWidth::Dword => (*value as u32).to_be_bytes().to_vec(),
                    ScalarWidth::Qword => (*value as u64).to_be_bytes().to_vec(),
                }));
            }
            DataDecl::Buffer { name, size } => {
                items.push(SInst::Label(format!("__data_{name}")));
                items.push(SInst::Bytes(vec![0u8; *size]));
            }
        }
    }
            ...
            SInst::CondBranch(reg, label) => {
                ...
                match arch {
                    "sh2" | "sh4" => {
                        ...
                        if disp < 0 || disp > 255 {
                            return Err("branch out of range for bt (0-255 words)".into());
                        }
                    ...
                    "m68k" => {
                        ...
                        bin.extend(
                            &(0x6700u16 | ((disp as u8) as u16 & 0xFF))
                                .to_le_bytes()
                                .to_vec(),
                        );
                    }
                ...
                match arch {
                    "sh2" | "sh4" => {
                        ...
                        if disp < -2048 || disp > 2047 {
                            return Err("branch out of range for bra".into());
                        }
                    ...
                    "m68k" => {
                        ...
                        bin.extend(
                            &(0x6000u16 | ((disp as u8) as u16 & 0xFF))
                                .to_le_bytes()
                                .to_vec(),
                        );
                    }

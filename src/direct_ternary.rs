...
impl DirectTernaryBuilder {
    pub fn build_bin(p: &Program, out: &Path) -> Result<TernaryBuildOutput, String> {
        if p.target != "ternary" {
            return Err(format!("need 'ternary', got '{}'", p.target));
        }
        let k = tassemble(p)?;
        std::fs::write(out, &k).map_err(|e| e.to_string())?;
        Ok(TernaryBuildOutput {
...
fn tencode(i: &TInst, off: u32, lm: &BTreeMap<String, u32>) -> Result<Vec<u8>, String> {
    ...
    Ok(match i {
        ...
        TInst::TAdd(d, s1, s2) => {
            ...
            trits.push(op[0]);
            trits.push(op[1]);
            trits.extend(reg2trits(*s1));
            trits.extend(reg2trits(*s2));
            trits.extend(reg2trits(*d));
            pack_trits(&trits)
        }
        TInst::TSub(d, s1, s2) => {
            let op = [1, -1]; // SUB = 1T
...
fn tparse(t: &str) -> Result<TInst, String> {
    ...
    if t.ends_with(':') {
        return Ok(TInst::Label(t[..t.len() - 1].to_string()));
    }
    let p: Vec<&str> = t
        .splitn(4, |c: char| c == ' ' || c == '\t')
        .filter(|s| !s.is_empty())
    ...
    match m {
        ...
        "mov" | "mv" if v.len() == 2 => {
            let d = gr(v[0])?;
            if let Ok(imm) = v[1].parse::<i64>() {
                Ok(TInst::TLi(d, imm))
            } else {
                Ok(TInst::TMov(d, gr(v[1])?))
            }
        }
    ...
    for b in &p.blocks {
        ...
        for l in &b.lines {
            ...
            if let Some(x) = t.strip_prefix("emit ") {
                items.push(TInst::TJmp(format!("__event_{}", x.trim())));
                continue;
            }
            if let Some(x) = t.strip_prefix("call ") {
                items.push(TInst::TJmp(format!("__proc_{}", x.trim())));
                continue;
            }
            if t == "ret" || t == "hlt" {
                items.push(TInst::THlt);
                continue;
            }

            // if <reg>
            if let Some(cond_str) = t.strip_prefix("if ") {
                let reg = trp(cond_str.trim())
            ...
            if let Some(cond_str) = t.strip_prefix("elif ") {
                ...
                if let TInst::TJz(ref mut l, _) = items[prev] {
                    *l = elif_lbl.clone();
                }
            ...
            match tparse(t) {
                Ok(i) => items.push(i),
                Err(_) => return Err(format!("ternary: '{}'", t)),
            }
        }
    }
        ...
        match d {
            DataDecl::String { name, value } => {
                ...
                for byte in &b {
                    ...
                    for _ in 0..6 {
                        // 6 trits per byte in balanced ternary
                        let t = ((v % 3 + 3) % 3) as i8;
                        trits.push(match t {
                            0 => 0,
                            1 => 1,

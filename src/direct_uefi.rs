    ...
    match mnemonic {
        "mov" => {
            ...
            if args[1].starts_with('[')
                || args[1].contains("__data")
                || args[1].contains("__event")
                || args[1].contains("__proc")
            {
                // mov reg, [label]
                let rd = parse_reg64(args[0])
            ...
            let rd =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
            let rs =
                parse_reg64(args[1]).ok_or_else(|| format!("unknown register '{}'", args[1]))?;
        ...
        "add" => {
            let args: Vec<&str> = rest
                ...
                .collect();
            if args.len() != 2 {
                return Err(format!("bad add syntax: {rest}"));
            }
            let rd =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
        ...
        "sub" => {
            let args: Vec<&str> = rest
                ...
                .collect();
            if args.len() != 2 {
                return Err(format!("bad sub syntax: {rest}"));
            }
            let rd =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
            if let Ok(imm) = args[1].parse::<i32>() {
        ...
        "inc" => {
            let rd = parse_reg64(rest.trim())
                .ok_or_else(|| format!("unknown register '{}'", rest.trim()))?;
            Ok(Inst::IncReg64(rd))
        }
        "dec" => {
            let rd = parse_reg64(rest.trim())
        ...
        "cmp" => {
            let args: Vec<&str> = rest
                ...
                .collect();
            if args.len() != 2 {
                return Err(format!("bad cmp syntax: {rest}"));
            }
            let ra =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
            if let Ok(imm) = args[1].parse::<i32>() {
        ...
        "lea" => {
            let args: Vec<&str> = rest
                ...
                .collect();
            if args.len() != 2 {
                return Err(format!("bad lea syntax: {rest}"));
            }
            let rd =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
            let label = args[1].trim_matches(|c| c == '[' || c == ']' || c == ' ');
        ...
        "test" => {
            let args: Vec<&str> = rest
                ...
                .collect();
            if args.len() != 2 {
                return Err(format!("bad test syntax: {rest}"));
            }
            let ra =
                parse_reg64(args[0]).ok_or_else(|| format!("unknown register '{}'", args[0]))?;
            let rb =
    ...
    F: FnOnce(Reg64, Reg64) -> Inst,
{
    let args: Vec<&str> = rest
        ...
        .collect();
    if args.len() != 2 {
        return Err(format!("expected 2 register arguments, got {}", args.len()));
    }
...
fn build_pe32_plus(kernel: &[u8], _program: &Program) -> Result<Vec<u8>, String> {
    ...
    let size_of_image =
        sect_align + ((kernel.len() as u32 + sect_align - 1) / sect_align) * sect_align;
...
impl DirectUefiAssembler {
    pub fn assemble(program: &Program) -> Result<Vec<u8>, String> {
        ...
        items.push(Inst::MovMem64Reg64(
            "__uefi_system_table".to_string(),
            Reg64::R8,
        ));
        // Call entry event
        ...
        items.push(Inst::MovReg64Mem64(
            Reg64::Rcx,
            "__uefi_system_table".to_string(),
        ));
        items.push(Inst::MovReg64Mem64(
            Reg64::Rdx,
            "__builtin_putc_wide".to_string(),
        ));
        //TODO: Desimplify
            ...
            for line in &block.lines {
                ...
                if let Some(cond_str) = trimmed.strip_prefix("if ") {
                    let reg = match cond_str.trim().parse::<i64>() {
                        ...
                        Err(_) => parse_reg64(cond_str.trim()).ok_or_else(|| {
        ...
        match item {
            ...
            _ => {
                ...
                offset += match item {
                    ...
                    Inst::XorReg64Reg64(_, _)
                    | Inst::AndReg64Reg64(_, _)
                    | Inst::OrReg64Reg64(_, _) => 3,
                    Inst::IncReg64(_) | Inst::DecReg64(_) => 3,
                    Inst::MulReg64(_) | Inst::DivReg64(_) => 3,
                    Inst::NotReg64(_) | Inst::NegReg64(_) => 3,
                    Inst::Push(_) | Inst::Pop(_) => {
                        1 + if reg64_num(match item {

...
impl DirectVmBuilder {
    pub fn build_bytecode(program: &Program, out_path: &Path) -> Result<VmBuildOutput, String> {
        if program.target != "vm" {
            return Err(format!(
                "vm backend requires target 'vm', got '{}'",
                program.target
            ));
        }
...
pub fn run_vm_bytecode(bytecode: &[u8]) -> Result<i64, String> {
    ...
    loop {
        ...
        match op {
            ...
            OP_DUP => {
                let v = *stack.last().ok_or("empty stack for dup")?;
                stack.push(v);
            }
            OP_DROP => {
                stack.pop().ok_or("empty stack for drop")?;
            }
            OP_SWAP => {
                let a = stack.pop().ok_or("empty stack")?;
                let b = stack.pop().ok_or("empty stack")?;
                stack.push(a);
                stack.push(b);
            }
            OP_ADD => {
                let a = stack.pop().ok_or("empty")?;
                let b = stack.pop().ok_or("empty")?;
                stack.push(b + a);
            }
            OP_SUB => {
                let a = stack.pop().ok_or("empty")?;
            ...
            OP_JZ => {
                ...
                ip += 4;
                if stack.pop().ok_or("empty stack")? == 0 {
                    ip = target;
                }
            }
            OP_JNZ => {
            ...
            OP_PRINT => {
                let v = stack.pop().ok_or("empty")?;
                print!("{}", v);
            }
            OP_PRINTC => {
                let v = stack.pop().ok_or("empty")? as u8;
                print!("{}", v as char);
            }
            OP_READ => {
            ...
            OP_PUSHS => {
                let start = ip;
                while ip < bytecode.len() && bytecode[ip] != 0 {
                    ip += 1;
                }

...
use decretum::{
    Arm7tdmiBuilder, Arm9Builder, BesmBuilder, BlackfinBuilder, C166Builder, Cdc6600Builder,
    Direct6502Builder, Direct6809Builder, DirectAarch64Builder, DirectAlphaBuilder,
    DirectArduinoEsp32Builder, DirectArmCmBuilder, DirectAvrBuilder, DirectBiosBuilder,
    DirectCheriBuilder, DirectElf32Builder, DirectElfBuilder, DirectIa64Builder, DirectM68kBuilder,
    DirectMachoBuilder, DirectMicroblazeBuilder, DirectMipsBuilder, DirectNios2Builder,
    DirectOpenriscBuilder, DirectPICBuilder, DirectPariscBuilder, DirectPpcBuilder,
    DirectQuantum8Builder, DirectQuantum64Builder, DirectRisCvCheriBuilder, DirectRiscvBuilder,
    DirectSh2Builder, DirectSparcBuilder, DirectTernaryBuilder, DirectUefiBuilder,
    DirectVliwBuilder, DirectVmBuilder, DirectWin32Builder, DirectX86_64Builder, DirectZ80Builder,
    DlxBuilder, ElbrusBuilder, FrBuilder, H8Builder, HarvardBuilder, Hp3000Builder, HuC6280Builder,
    I4004Builder, I8008Builder, I8080Builder, I8086Builder, JovialBuilder, Lc3Builder, M16cBuilder,
    M6800Builder, Mico32Builder, Mil1750aBuilder, MillBuilder, MirBuilder, MmixBuilder,
    Mos6501Builder, Msp430Builder, NativeStackBuilder, Nec78kBuilder, NecV20Builder, Parser,
    Pdp8Builder, Pdp11Builder, PicoblazeBuilder, PortableBuilder, Ppc740Builder, Ppc970Builder,
    R8cBuilder, Rl78Builder, RxBuilder, S360Builder, SharcBuilder, Tms320Builder, UnivacBuilder,
    UralBuilder, V810Builder, VaxBuilder, Xc800Builder, ZArchBuilder,
};
    ...
    match cli.command {
        ...
        Commands::CompileArch { inputs, out_file } => {
            ...
            match program.target.as_str() {
                ...
                "s360" => {
                    let o = S360Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "zarch" => {
                    let o = ZArchBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "univac" => {
                    let o = UnivacBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "cdc6600" => {
                    let o = Cdc6600Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "pdp8" => {
                    let o = Pdp8Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "pdp11" => {
                    let o = Pdp11Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "vax" => {
                    let o = VaxBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "hp3000" => {
                    let o = Hp3000Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "i4004" => {
                    let o = I4004Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "i8008" => {
                    let o = I8008Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "i8080" => {
                    let o = I8080Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "i8086" => {
                    let o = I8086Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "m6800" => {
                    let o = M6800Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mos6501" => {
                    let o = Mos6501Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "tms320" => {
                    let o = Tms320Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "blackfin" => {
                    let o = BlackfinBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "sharc" => {
                    let o = SharcBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "c166" => {
                    let o = C166Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "xc800" => {
                    let o = Xc800Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "rl78" => {
                    let o = Rl78Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "rx" => {
                    let o = RxBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "h8" => {
                    let o = H8Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "msp430" => {
                    let o = Msp430Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "v20" => {
                    let o = NecV20Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "nec78k" => {
                    let o = Nec78kBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "m16c" => {
                    let o = M16cBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "r8c" => {
                    let o = R8cBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "fr" => {
                    let o = FrBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mico32" => {
                    let o = Mico32Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "picoblaze" => {
                    let o = PicoblazeBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mmix" => {
                    let o = MmixBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "dlx" => {
                    let o = DlxBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "lc3" => {
                    let o = Lc3Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "huc6280" => {
                    let o = HuC6280Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "v810" => {
                    let o = V810Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "arm9" => {
                    let o = Arm9Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "armcm" => {
                    let o = DirectArmCmBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "arm7tdmi" => {
                    let o = Arm7tdmiBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "ppc740" => {
                    let o = Ppc740Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "ppc970" => {
                    let o = Ppc970Builder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mil1750a" => {
                    let o = Mil1750aBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "jovial" => {
                    let o = JovialBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "ural" => {
                    let o = UralBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "besm" => {
                    let o = BesmBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "elbrus" => {
                    let o = ElbrusBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mir" => {
                    let o = MirBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "harvard" => {
                    let o = HarvardBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "mill" => {
                    let o = MillBuilder::build_bin(&program, &out_file)?;
                    println!("wrote {}", o.bin_path.display());
                }
                "alpha" => {
                ...
                "arduino_nano_esp32" => {
                    let o = DirectArduinoEsp32Builder::build_app(&program, &out_file)?;
                    println!("wrote {}", o.app_path.display());
                }
                _ => {
                    return Err(format!(

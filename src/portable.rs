...
fn compile_program(program: &Program) -> Result<BytecodeProgram, String> {
    if program.target == "bios16"
        || program.target == "uefi"
        || program.target == "armcm"
        || program.target == "riscv"
        || program.target == "x86_64"
        || program.target == "riscv64"
        || program.target == "aarch64"
        || program.target == "macho"
        || program.target == "elf64"
        || program.target == "cheri"
        || program.target == "riscv_cheri"
        || program.target == "win32"
        || program.target == "elf32"
        || program.target == "mips"
        || program.target == "ppc"
        || program.target == "sparc"
        || program.target == "alpha"
        || program.target == "parisc"
        || program.target == "openrisc"
        || program.target == "nios2"
        || program.target == "microblaze"
        || program.target == "6502"
        || program.target == "z80"
        || program.target == "6809"
        || program.target == "pic"
        ...
        || program.target == "quantum8"
        || program.target == "quantum64"
        || program.target == "ia64"
        || program.target == "vliw"
        || program.target == "s360"
        || program.target == "zarch"
        || program.target == "univac"
        || program.target == "cdc6600"
        || program.target == "pdp8"
        || program.target == "pdp11"
        || program.target == "vax"
        || program.target == "hp3000"
        || program.target == "i4004"
        || program.target == "i8008"
        || program.target == "i8080"
        || program.target == "i8086"
        || program.target == "m6800"
        || program.target == "mos6501"
        || program.target == "tms320"
        || program.target == "blackfin"
        || program.target == "sharc"
        || program.target == "c166"
        || program.target == "xc800"
        || program.target == "rl78"
        || program.target == "rx"
        || program.target == "h8"
        || program.target == "msp430"
        || program.target == "v20"
        || program.target == "nec78k"
        || program.target == "m16c"
        || program.target == "r8c"
        || program.target == "fr"
        || program.target == "mico32"
        || program.target == "picoblaze"
        || program.target == "mmix"
        || program.target == "dlx"
        || program.target == "lc3"
        || program.target == "huc6280"
        || program.target == "v810"
        || program.target == "arm7tdmi"
        || program.target == "arm9"
        || program.target == "ppc740"
        || program.target == "ppc970"
        || program.target == "mil1750a"
        || program.target == "jovial"
        || program.target == "ural"
        || program.target == "besm"
        || program.target == "elbrus"
        || program.target == "mir"
        || program.target == "harvard"
        || program.target == "mill"
    {
        return Err(format!(
...
fn compile_block(block: &Block) -> Result<BytecodeBlock, String> {
    ...
    for line in &block.lines {
        ...
        if let Some(cond_str) = trimmed.strip_prefix("if ") {
            ...
            cf_counter += 1;
            raw_ops.push(RawOp {
                op: RawOpKind::Final(Op::Cmp(cond.clone(), Operand::Imm(0))),
            });
            let je_idx = raw_ops.len();
            raw_ops.push(RawOp {
                op: RawOpKind::JumpIf(Cond::Eq, endif_lbl.clone()),
            });
        ...
        if let Some(cond_str) = trimmed.strip_prefix("elif ") {
            ...
            if let Some(RawOp {
                op: RawOpKind::JumpIf(_, label),
            }) = raw_ops.get_mut(*prev_je)
            {
                *label = elif_lbl.clone();
            }
            // jmp past the elif chain to endif
            raw_ops.push(RawOp {
                op: RawOpKind::Jump(frame.endif_label.clone()),
            });
            // Emit the elif label
            ...
            let je_idx = raw_ops.len();
            raw_ops.push(RawOp {
                op: RawOpKind::JumpIf(Cond::Eq, frame.endif_label.clone()),
            });
        ...
        if trimmed == "else" {
            ...
            if let Some(RawOp {
                op: RawOpKind::JumpIf(_, label),
            }) = raw_ops.get_mut(*frame.je_indices.last().ok_or("internal: no je indices")?)
            {
            ...
            if labels
                .insert(frame.else_label.clone(), raw_ops.len())
                .is_some()
            {
        ...
        if trimmed == "endif" {
            ...
            if labels
                .insert(frame.endif_label.clone(), raw_ops.len())
                .is_some()
            {
        ...
        if let Some(cond_str) = trimmed.strip_prefix("while ") {
            ...
            let je_idx = raw_ops.len();
            raw_ops.push(RawOp {
                op: RawOpKind::JumpIf(Cond::Eq, endwhile_lbl.clone()),
            });
            cf_stack.push(CfFrame {
                kind: CfKind::While,
...
fn parse_line_to_raw(line: &str, scope: &str) -> Result<RawOp, String> {
    ...
    if let Some(rest) = line.strip_prefix("random_range ") {
        ...
        return Ok(RawOp {
            op: RawOpKind::Final(Op::RandomRange(
                parse_dest(dst)?,
                parse_operand(lo)?,
                parse_operand(hi)?,
            )),
